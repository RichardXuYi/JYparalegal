...
function SidebarFooter({ collapsed }: { collapsed: boolean }) {
  ...
  if (collapsed) {
    return (
      <div className="mt-auto flex shrink-0 flex-col items-center gap-1 p-2">
        <button
          type="button"
          title={t('sidebar.settings')}
          aria-label={t('sidebar.settings')}
          onClick={() => openSettings()}
          className="flex h-8 w-8 items-center justify-center rounded-lg text-white/70 hover:bg-white/15 hover:text-white"
        >
...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {!sidebarCollapsed && isChatPage && (
        <div className="shell-glass mx-0.5 mt-2 flex shrink-0 items-center gap-2 rounded-lg px-2.5 py-1.5">
      ...
      {!sidebarCollapsed && isSigningSection && (
        <div className="mt-2 min-h-0 flex-1 overflow-hidden">
          <SigningSidebar embedded />
        </div>
      )}

      {/* Session list — chat only, below the platform nav */}
      {!sidebarCollapsed && isChatPage && sessions.length > 0 && (
