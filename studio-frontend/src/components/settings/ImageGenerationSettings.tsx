...
export function ImageGenerationSettings() {
  ...
  useEffect(() => {
    // load() flips `loading` synchronously before its first await; defer to a
    // microtask so the state write happens outside the effect body.
    let cancelled = false;
    queueMicrotask(() => {
  ...
  return (
    <div data-testid="image-generation-settings" className="space-y-6">
      ...
      {loading ? (
        ...
        <SettingsGroup padded gradientClass="gradient-updates">
