    ...
    return (
      <div className="space-y-1 overflow-y-auto">
        <div className="px-3 py-2 text-2xs uppercase tracking-wide text-muted-foreground">
          ...
          {agent?.name ? <span className="ml-1 text-foreground/60">路 {agent.name}</span> : null}
        </div>
        <FileTreeNodeList
      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          <h2 className="truncate text-sm font-semibold">
            {t('workspace.title', 'Workspace')}
            {agent?.name ? <span className="ml-2 font-normal text-foreground/70">路 {agent.name}</span> : null}
          </h2>
          {workspace && !compact ? (
