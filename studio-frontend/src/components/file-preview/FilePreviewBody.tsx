 ...
 * disk. Edit tools show the exact old 鈫?new snippet swap. Write-family
...
export function FilePreviewBody({
  ...
  // Reset load state during render when the load inputs change (React-recommended
  // adjust-during-render pattern) instead of setState-in-effect; the effect below
  // only performs the async disk reads.
  const [prevLoadKey, setPrevLoadKey] = useState<{
    file: FilePreviewBodyProps['file'];
    enforcedReadOnly: boolean;
    mode: FilePreviewBodyProps['mode'];
    tabs: Tab[];
    unsupportedPreviewFormat: boolean;
    isRichDocumentPreview: boolean;
  } | null>(null);
  const loadKeyChanged =
    !prevLoadKey
    || prevLoadKey.file !== file
    || prevLoadKey.enforcedReadOnly !== enforcedReadOnly
    || prevLoadKey.mode !== mode
    || prevLoadKey.tabs !== tabs
    || prevLoadKey.unsupportedPreviewFormat !== unsupportedPreviewFormat
    || prevLoadKey.isRichDocumentPreview !== isRichDocumentPreview;
  if (loadKeyChanged) {
    setPrevLoadKey({ file, enforcedReadOnly, mode, tabs, unsupportedPreviewFormat, isRichDocumentPreview });
    ...
    if (mode === 'diff') {
      ...
      setDraft(null);
    } else if (unsupportedPreviewFormat) {
      setState({ status: 'ready', content: '', readOnly: enforcedReadOnly });
      setDraft(null);
    } else if (isRichDocumentPreview) {
    ...
    } else if (file.contentType === 'snapshot' || file.contentType === 'video' || file.contentType === 'audio') {
      setState({ status: 'ready', content: '', readOnly: enforcedReadOnly });
      setDraft(null);
    } else {
      setState({ status: 'loading' });
        ...
        <p className="max-w-md text-xs leading-relaxed text-muted-foreground">
          {directOpen
            ? t('filePreview.errors.largeBinaryOpenHint', {
              defaultValue: 'This file is {{size}}. JYparalegal does not provide an inline preview for it. You can confirm to open it directly in your system default app.',
          ...
          <div className="space-y-1.5">
            ...
            <p className="max-w-md text-xs leading-relaxed text-muted-foreground">
              {t(
                ...
                'JYparalegal cannot read this path. The file may have been moved, deleted, or may not be accessible to the current account. You can inspect it in your file manager.',
              )}
            </p>
          </div>
          <Button variant="outline" size="sm" onClick={handleOpenInFinder}>
