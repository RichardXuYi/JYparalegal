 ...
 * happens in the Electron main process; this store only talks to it through
 * `hostApi.auth.*` (invokeHost). Tokens live in the main process, so this
  ...
  persist(
    (set) => ({
      ...
      logout: async () => {
        ...
        // Safety net for the web build: the bridge disconnects inside its
        // logout HTTP path; on the desktop this global is absent and this is
        // a no-op (the main process switches the Gateway scope itself).
