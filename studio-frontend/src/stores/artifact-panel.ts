 ...
 * active (鍙樻洿 / 棰勮 / 宸ヤ綔绌洪棿), the focused file shared across the
 * 鍙樻洿 and 棰勮 tabs, and the open/close state.
 *
...
interface ArtifactPanelState {
   ...
   * 鍙樻洿 tab (drives the right-pane diff) and the 棰勮 tab (drives the
  ...
  /** Drop the panel's per-user UI state (kept width stays). */
...
export const useArtifactPanel = create<ArtifactPanelState>()(
  persist(
    (set, get) => ({
      ...
      resetForUserSwitch: () => set({ open: false, tab: 'changes', focusedFile: null }),
    }),
    {
      name: 'grandpoem-studio.artifact-panel',
