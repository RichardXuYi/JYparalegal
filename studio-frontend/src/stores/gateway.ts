...
// Bumped by resetForUserSwitch so an init that started for the PREVIOUS
// account can never write its results into the new account's state.
let gatewayInitGeneration = 0;
...
interface GatewayState {
  ...
  clearError: () => void;
  /** Drop all state so the next account re-inits against its own worker. */
...
export const useGatewayStore = create<GatewayState>((set, get) => ({
  ...
  init: async () => {
    ...
    const generation = ++gatewayInitGeneration;
    ...
    promiseRef.current = (async () => {
      try {
        ...
        if (generation === gatewayInitGeneration) {
          set({ lastError: String(error) });
        }
      } finally {
        if (gatewayInitPromise === promiseRef.current) gatewayInitPromise = null;
      }
    })();
    gatewayInitPromise = promiseRef.current;
  ...
  resetForUserSwitch: () => {
    // Re-init must run again for the new account's worker; the reconcile
    // timer stays registered (it re-polls status every 30s) but any stale
    // dedupe entries must not suppress the new account's first events.
    // Bumping the generation also invalidates an init that was still in
    // flight for the previous account.
    gatewayInitGeneration += 1;
    gatewayEventDedupe.clear();
