        ...
        case 'final': {
          ...
          if (finalMsg) {
            ...
            set((s) => {
              ...
              const attachedGatewayUrls = new Set(
                (normalizedFinalMessage._attachedFiles || [])
                  .map(f => f.gatewayUrl).filter(Boolean) as string[],
              );
              ...
              // Collect media URLs from runtime assistant.delta events
              const runStateMediaUrls = s.runtimeRuns[runId]?.mediaUrls ?? [];
              for (const url of runStateMediaUrls) {
                if (attachedGatewayUrls.has(url)) continue;
