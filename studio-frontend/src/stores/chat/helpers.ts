...
// 鈹€鈹€ Local image cache 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
function extractRawFilePaths(text: string): Array<{ filePath: string; mimeType: string }> {
  ...
  // Windows absolute paths (C:\... or C:/... D:\...) —lookbehind rejects drive letter glued to a word
  ...
  for (const block of content as ContentBlock[]) {
    if (block.type === 'image') {
      ...
      // preview is filled in later by `loadMissingPreviews` -> Main proxy.
        ...
        // Derive mimeType and fileName from the URL if not provided by Gateway.
        // Gateway sends non-image files (e.g. .html) via /__openclaw__/assistant-media?source=<path>
        // but may not set mimeType/alt fields correctly.
        let mimeType = block.mimeType;
        let fileName = typeof block.alt === 'string' && block.alt ? block.alt : undefined;
        if (!mimeType || !fileName) {
          try {
            const url = new URL(block.url, 'http://localhost');
 ...
 * text is internal narration (e.g. "鐢熸垚涓紝绋嶇瓑" + `image_generate`). Those
