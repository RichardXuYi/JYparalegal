...
function StepDetailCard({ step, visible, isParent, parentExpanded, onToggleParent }: {
  step: TaskStep;
  /** Whether this step should be rendered (controlled by parent's expand state). */
  visible: boolean;
  /** Whether this step has child steps. */
  isParent: boolean;
  /** Whether this parent step is currently expanded (only relevant when isParent). */
  parentExpanded?: boolean;
  /** Toggle this parent step's expand state (only called when isParent). */
  onToggleParent?: () => void;
}) {
  ...
  // A step is expandable if it has its own detail, or if it's a parent with children.
  const canExpand = hasDetail || (isParent && !parentExpanded);
    ...
    >
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          {step.detail && !expanded && !isFlatRow && !isParent && (
            <p
              className={cn(
        ...
        {canExpand && (
          <span className="mt-0.5 shrink-0 text-muted-foreground">
            {isParent
              ? (parentExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />)
              : (expanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />)
            }
          </span>
        )}
      </button>
      {step.detail && (expanded || (isParent && parentExpanded)) && canExpand && isFlatRow && (() => {
          ...
          {step.detail && (expanded || (isParent && parentExpanded)) && canExpand && (isNarration || isThinking) && (
            <div className="mt-3 rounded-lg border border-black/10 bg-black/[0.03] px-3 py-2 dark:border-white/10 dark:bg-white/[0.03]">
              <pre
                className="whitespace-pre-wrap break-words text-xs leading-5 text-muted-foreground"
              >
...
export function ExecutionGraphCard({
  ...
  // Track which parent steps (steps that have children) are expanded.
  // Children are only visible when their parent is expanded.
  const [expandedParents, setExpandedParents] = useState<Set<string>>(new Set());
  const parentIds = new Set(steps.filter((s) => s.parentId).map((s) => s.parentId));
  const isParentMap = new Map(steps.map((s) => [s.id, parentIds.has(s.id)]));

  const toggleParent = (stepId: string) => {
  ...
  const isStepVisible = (step: TaskStep): boolean => {
    if (!step.parentId) return true;
    return expandedParents.has(step.parentId);
  };
    ...
    >
      ...
      <div className="mt-0 px-0 py-0">
        ...
        {steps.map((step) => {
          ...
          const stepIsParent = isParentMap.get(step.id) ?? false;
          const stepVisible = isStepVisible(step);
          ...
          <div key={step.id} className="mt-0.5">
            ...
            >
              ...
              <StepDetailCard
                step={step}
                visible={stepVisible}
                isParent={stepIsParent}
                parentExpanded={stepIsParent ? expandedParents.has(step.id) : undefined}
                onToggleParent={stepIsParent ? () => toggleParent(step.id) : undefined}
              />
            </div>
          </div>
        )})}
