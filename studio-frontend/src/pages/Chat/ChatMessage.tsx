...
import { Sparkles, Copy, Check, Wrench, FileText, Film, Music, FileArchive, File, X, FolderOpen, ZoomIn, Loader2, CheckCircle2, AlertCircle, ExternalLink } from 'lucide-react';
...
function isUserFacingAttachmentWhenFolded(file: AttachedFileMeta): boolean {
  ...
  // HTML/Markdown files are always user-facing, regardless of source.
  // This covers:
  //   - message-ref: paths parsed from the assistant reply (e.g. "MEDIA:/workspace/demo.html")
  //   - gateway-media: files sent via Gateway's assistant-media URL (e.g. .html files
  //     that Gateway sends as /__openclaw__/assistant-media?source=... because the
  //     renderer cannot open local paths directly)
  if (isHtmlOrMarkdownPreview(file)) return true;
  return false;
}
...
function extractPreviewDocumentPaths(text: string): AttachedFileMeta[] {
  ...
  const taggedRegex = new RegExp(`(?:^|[\\s(\\[{>])(?:MEDIA|media):((?:\\/|~\\/|[A-Za-z]:[\\\\/])[^\\s\\n"'()\\[\\],<>]*?\\.(?:${exts}))`, 'g');
  ...
  for (const regex of [unixRegex, winRegex, skillMarkdownRegex, skillDirRegex]) {
...
function parseEmbedAttributes(attrString: string): Record<string, string> {
...
function extractEmbeds(text: string): { cleanedText: string; embeds: EmbedInfo[] } {
  const embeds: EmbedInfo[] = [];
  const regex = /\[embed\s+([^\]]*?)\s*\/\]/g;
  const cleanedText = text.replace(regex, (_match, attrString: string) => {
    const attrs = parseEmbedAttributes(attrString);
    const url = attrs.url || (attrs.ref ? `/__openclaw__/canvas/documents/${attrs.ref}/index.html` : '');
    if (!url) return '';
    embeds.push({ url, title: attrs.title || '打开预览' });
    return '';
  });
  return { cleanedText, embeds };
}

function EmbedLinkCard({ embed }: { embed: EmbedInfo }) {
  const handleOpen = useCallback(() => {
    void hostApi.shell.openExternal(embed.url);
  }, [embed.url]);
  return (
    <button
      type="button"
      onClick={handleOpen}
      className="group flex items-center gap-3 w-full rounded-xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 px-4 py-3 hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-left"
    >
      <ExternalLink className="h-4 w-4 text-muted-foreground shrink-0 group-hover:text-primary transition-colors" />
      <div className="min-w-0 flex-1 overflow-hidden">
        <div className="text-sm font-medium text-foreground truncate">{embed.title}</div>
        <div className="text-xs text-muted-foreground truncate">{embed.url}</div>
      </div>
    </button>
  );
}
...
export const ChatMessage = memo(function ChatMessage({
  ...
  // Extract [embed ...] blocks from assistant messages for click-to-open cards
  const { cleanedText: embedCleanedText, embeds: assistantEmbeds } = !isUser ? extractEmbeds(text) : { cleanedText: text, embeds: [] as EmbedInfo[] };
  const displayText = !isUser ? embedCleanedText : text;
  ...
  const hasText = !hideAssistantText && displayText.trim().length > 0;
  ...
  const existingDerivedAttachedFiles = derivedAttachedFiles.filter((file) => {
    ...
    // For tool-result and gateway-media files, show them even before validation
    // completes — the gateway may still be writing them to disk asynchronously.
    // Without this, files vanish from the chat until the stat check returns,
    // and if the file never lands (e.g. transient path), they stay hidden forever.
    if (
      (file.source === 'tool-result' || file.source === 'gateway-media')
      && validatedPaths[file.filePath] === undefined
    ) return true;
  ...
  const processVisibleAttachments = filteredProcessAttachments.filter((file) => {
    const kind = validationKindForAttachment(file);
    if (!kind || !file.filePath) return true;
    // Same relaxation as existingDerivedAttachedFiles: show tool-result /
    // gateway-media files before validation completes so they don't vanish.
    if (
      (file.source === 'tool-result' || file.source === 'gateway-media')
      && validatedPaths[file.filePath] === undefined
    ) return true;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Embed link cards ([embed ...] → click-to-open in default browser) */}
        {!isUser && assistantEmbeds.length > 0 && (
          <div className="flex flex-col gap-2 w-full">
            {assistantEmbeds.map((embed, i) => (
              <EmbedLinkCard key={`embed-${i}`} embed={embed} />
            ))}
          </div>
        )}
        ...
        {hasText && (
          isUser ? (
            <UserMessageBubble text={displayText} />
          ) : (
            <AssistantMarkdown text={displayText} isStreaming={isStreaming} />
          )
        )}
...
function UserMessageBubble({
  ...
  return (
    <div className="relative rounded-2xl rounded-tr-sm bg-gradient-to-br from-indigo-500 to-violet-600 dark:from-orange-500 dark:to-red-600 px-4 py-3 text-white shadow-sm">
...
function FileCard({ file, onOpen }: { file: AttachedFileMeta; onOpen?: (file: AttachedFileMeta) => void }) {
  // File is clickable if it has a local path OR a gateway URL
  const isClickable = !!(file.filePath || file.gatewayUrl);
  const handleOpen = useCallback(() => {
    if (!isClickable) return;
    if (onOpen) {
      // Delegate to parent (handles both filePath and gatewayUrl)
      onOpen(file);
    } else if (file.filePath) {
      // Fallback: open directly via shell (only works with local path)
      void hostApi.shell.openPath(file.filePath);
    }
  }, [file, onOpen, isClickable]);
  ...
  return (
    <div 
      className={cn(
        ...
        isClickable && "cursor-pointer hover:bg-black/10 dark:hover:bg-white/10 transition-colors"
      )}
      onClick={handleOpen}
      title={isClickable ? "Open file" : undefined}
    >
