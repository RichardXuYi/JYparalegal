...
export function Skills() {
  ...
  // Opening the store prepares SkillHub on demand (kit download + managed Python).
  useEffect(() => {
    if (!installSheetOpen) return;
    void prepareStore();
  }, [installSheetOpen, prepareStore]);

  useEffect(() => {
    if (!installSheetOpen || !storeReady) {
      return;
    }
    ...
    return () => clearTimeout(timer);
  }, [installQuery, installSheetOpen, storeReady, searchSkills]);
      ...
      <Sheet open={installSheetOpen} onOpenChange={setInstallSheetOpen}>
        ...
        >
          <div className="px-7 py-6 border-b border-black/10 dark:border-white/10">
            ...
            {storeReady && !storePreparing && !storeError && (
          ...
          <div className="flex-1 overflow-y-auto px-6 py-4">
            {storePreparing && (
              <div className="flex flex-col items-center justify-center py-20 text-center text-muted-foreground">
                <LoadingSpinner size="lg" />
                <p className="mt-4 text-sm">{t('marketplace.preparing')}</p>
                <p className="mt-1 text-xs text-muted-foreground/70">{t('marketplace.preparingHint')}</p>
              </div>
            )}

            {!storePreparing && storeError && (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                <AlertCircle className="h-10 w-10 mb-4 text-destructive/70" />
                <p className="text-sm font-medium text-destructive">{t('marketplace.prepareFailed')}</p>
                <p className="mt-2 max-w-sm text-xs text-muted-foreground break-all">{storeError}</p>
                <Button variant="outline" size="sm" className="mt-4" onClick={() => void prepareStore()}>
            ...
            {!searching && searchResults.length > 0 && (
              <div className="flex flex-col gap-1">
                {searchResults.map((skill) => {
                  // SkillHub installs land in a folder named after the trailing
                  // slug segment ('@handle/pdf' -> 'pdf'), which is what the
                  // scanner reports back as the skill id/slug.
                  const slugTail = skill.slug.includes('/')
                    ? skill.slug.slice(skill.slug.lastIndexOf('/') + 1)
                    : skill.slug;
                  const isInstalled = safeSkills.some((s) =>
                    [s.id, s.slug].includes(skill.slug)
                    || [s.id, s.slug].includes(slugTail)
                    || s.name === skill.name,
                  );
                  ...
                  return (
                    <div
                      key={skill.slug}
                      className="group flex flex-row items-center justify-between py-3.5 px-3 rounded-xl border-b border-black/5 dark:border-white/5 last:border-0"
                    >
