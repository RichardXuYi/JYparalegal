...
function TaskDialog({ open, job, configuredChannels, onClose, onSave }: TaskDialogProps) {
  ...
  // from a previous agent are not offered for insertion. Adjust during render
  ...
  useEffect(() => {
    ...
    // loadQuickSkills flips loading state synchronously before its first
    // await; defer to a microtask so the writes stay out of the effect body.
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) void loadQuickSkills();
    });
    return () => {
      cancelled = true;
    };
  ...
  // setState-in-effect; the effect below only performs the async target load.
  const [prevAccountKey, setPrevAccountKey] = useState<{
    deliveryMode: typeof deliveryMode;
    defaultAccountId: string | undefined;
    selectedDeliveryAccountId: string;
  } | null>(null);
  const accountKeyChanged =
    !prevAccountKey
    || prevAccountKey.deliveryMode !== deliveryMode
    || prevAccountKey.defaultAccountId !== selectedChannel?.defaultAccountId
    || prevAccountKey.selectedDeliveryAccountId !== selectedDeliveryAccountId;
  if (accountKeyChanged) {
    setPrevAccountKey({
      deliveryMode,
      defaultAccountId: selectedChannel?.defaultAccountId,
      selectedDeliveryAccountId,
    });
    if (deliveryMode !== 'announce') {
      setSelectedDeliveryAccountId('');
    } else if (!selectedDeliveryAccountId && selectedChannel?.defaultAccountId) {
      setSelectedDeliveryAccountId(selectedChannel.defaultAccountId);
    }
  }

  const [prevTargetsKey, setPrevTargetsKey] = useState<{
    deliveryMode: typeof deliveryMode;
    effectiveDeliveryChannel: string;
    selectedResolvedAccountId: string | undefined;
    showsAccountSelector: boolean;
    unsupportedDeliveryChannel: boolean;
  } | null>(null);
  const targetsKeyChanged =
    !prevTargetsKey
    || prevTargetsKey.deliveryMode !== deliveryMode
    || prevTargetsKey.effectiveDeliveryChannel !== effectiveDeliveryChannel
    || prevTargetsKey.selectedResolvedAccountId !== selectedResolvedAccountId
    || prevTargetsKey.showsAccountSelector !== showsAccountSelector
    || prevTargetsKey.unsupportedDeliveryChannel !== unsupportedDeliveryChannel;
  if (targetsKeyChanged) {
    setPrevTargetsKey({
      deliveryMode,
    ...
    if (
      ...
      || !effectiveDeliveryChannel
      || unsupportedDeliveryChannel
      || (showsAccountSelector && !selectedResolvedAccountId)
    ) {
      setChannelTargetOptions([]);
      setLoadingChannelTargets(false);
    } else {
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent className="w-[calc(100%-2rem)] max-w-lg max-h-[90vh] flex flex-col rounded-3xl border-0 shadow-2xl bg-surface-modal">
...
export function Cron() {
  ...
  useEffect(() => {
    // fetchConfiguredChannels writes state from its async continuation; defer
    // the kick-off to a microtask to keep the effect body free of state writes.
    let cancelled = false;
    queueMicrotask(() => {
      if (!cancelled) void fetchConfiguredChannels();
    });
    return () => {
      cancelled = true;
    };
  ...
  if (loading) {
    return (
      <div data-testid="cron-page" className="flex flex-col h-full w-full dark:bg-background items-center justify-center">
  ...
  return (
    <div data-testid="cron-page" className="flex flex-col h-full w-full dark:bg-background overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full p-6 pt-8">
        ...
        <div className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 -mr-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="p-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-transparent flex flex-col justify-between min-h-[130px] relative overflow-hidden group hover:bg-black/10 dark:hover:bg-white/10 transition-colors">
              ...
              <div className="mt-4 flex items-baseline gap-3">
                <p className="text-stat font-bold">{safeJobs.length}</p>
            ...
            <div className="p-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-transparent flex flex-col justify-between min-h-[130px] relative overflow-hidden group hover:bg-black/10 dark:hover:bg-white/10 transition-colors">
              ...
              <div className="mt-4 flex items-baseline gap-3">
                <p className="text-stat font-bold">{activeJobs.length}</p>
            ...
            <div className="p-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-transparent flex flex-col justify-between min-h-[130px] relative overflow-hidden group hover:bg-black/10 dark:hover:bg-white/10 transition-colors">
              ...
              <div className="mt-4 flex items-baseline gap-3">
                <p className="text-stat font-bold">{pausedJobs.length}</p>
            ...
            <div className="p-5 rounded-2xl bg-black/5 dark:bg-white/5 border border-transparent flex flex-col justify-between min-h-[130px] relative overflow-hidden group hover:bg-black/10 dark:hover:bg-white/10 transition-colors">
              ...
              <div className="mt-4 flex items-baseline gap-3">
                <p className="text-stat font-bold">{failedJobs.length}</p>
