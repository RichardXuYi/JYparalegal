...
import { ChevronRight, FileText, LayoutDashboard, LogOut, Monitor, Scale, Settings as SettingsIcon } from 'lucide-react';
...
export function More() {
  ...
  return (
    <div className="h-full overflow-y-auto overscroll-contain">
      ...
      <div className="mx-4 mt-4 overflow-hidden rounded-2xl bg-black/[0.03] dark:bg-white/[0.04] divide-y divide-black/5 dark:divide-white/5">
        <MoreRow icon={<LayoutDashboard strokeWidth={2} />} label="总览" onClick={() => navigate('/overview')} testId="more-overview" />
        <MoreRow icon={<FileText strokeWidth={2} />} label="签署" onClick={() => navigate('/signing')} testId="more-signing" />
        <MoreRow icon={<Scale strokeWidth={2} />} label="模拟法庭" onClick={() => navigate('/moot')} testId="more-moot" />
        <MoreRow icon={<FileText strokeWidth={2} />} label="模板" onClick={() => navigate('/templates')} testId="more-templates" />
        <MoreRow icon={<FileText strokeWidth={2} />} label="证据" onClick={() => navigate('/evidence')} testId="more-evidence" />
        <MoreRow icon={<FileText strokeWidth={2} />} label="企业管理" onClick={() => navigate('/company')} testId="more-company" />
      </div>
