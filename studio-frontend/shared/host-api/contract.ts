...
// ==================== Auth (cross-platform backend login) ====================
...
export type AuthLoginPayload = { username: string; password: string; rememberMe?: boolean };
...
export type AuthDeviceListResult = HostSuccess & { devices?: AuthDevice[] };
export type AuthRevokeDevicePayload = { deviceId: number };
...
/** Model-row metadata payloads (context window / thinking support). */
export type ProviderModelMetaPayload = { providerKey: string; modelId: string };
export type ProviderModelMetaUpdatePayload = ProviderModelMetaPayload & {
  contextWindow?: number;
  reasoning?: boolean;
};
export type ProviderModelMetaResult = {
  contextWindow?: number;
...
/**
 * Reference to a file attachment produced by a tool / agent in a specific
 * session and generation. ClawX-style revalidation: every access checks
 * sessionKey + generation against the current session state.
 */
...
export type ResolveWorkspaceContextResult =
  | { ok: true; workspaceRoot: string; executionCwd: string }
  | { ok: false; error: 'outsideSandbox' | 'notDirectory' | 'operationFailed' | string };
...
export type MediaResolveGatewayUrlPayload = { gatewayUrl: string };
export type MediaResolveGatewayUrlResult = {
...
export type SkillSyncStatusResult = HostSuccess & {
  requiresAuth?: boolean;
  items?: SkillSyncPlanItem[];
};
export type SkillSyncResult = HostSuccess & {
  requiresAuth?: boolean;
  pushed?: number;
  pulled?: number;
  skipped?: number;
...
export type SkillUploadErrorCode = 'invalid_skill' | 'exists' | 'too_large' | 'unsupported_type' | 'zip_unsafe';
export type SkillUploadResult = HostSuccess & {
  slug?: string;
  code?: SkillUploadErrorCode;
  error?: string;
};

// ---- Cross-platform config sync (agents/skills/preferences <-> backend) ----
// Unlike skill sync (bidirectional last-write-wins), config sync is explicitly
// directional: the UI offers "upload to cloud" and "download from cloud" so the
// user always knows which side wins. Sensitive credentials (provider API keys,
// OAuth tokens) are never included in any scope.
export type SyncScope = 'agents' | 'skills' | 'preferences';
...
export type SyncStatusPayload = { scopes?: SyncScope[] };
export type SyncStatusResult = HostSuccess & {
  requiresAuth?: boolean;
  scopes?: SyncScopeStatus[];
};
export type SyncRunPayload = { scopes: SyncScope[] };
export type SyncScopeOutcome = {
  scope: SyncScope;
  ok: boolean;
  pushed?: number;
  pulled?: number;
...
export type SyncRunResult = HostSuccess & {
  requiresAuth?: boolean;
  results?: SyncScopeOutcome[];
};
...
export type HostApiContract = {
  ...
  providers: {
    ...
    getModelsMeta: (payload: ProviderModelMetaPayload) => ProviderModelMetaResult;
    updateModelsMeta: (payload: ProviderModelMetaUpdatePayload) => HostSuccess;
  };
  files: {
    ...
    resolveWorkspaceContext: (payload: WorkspaceContextInput) => ResolveWorkspaceContextResult;
    statWorkspaceFile: (payload: WorkspaceFileRef) => StatFileResult;
    readWorkspaceText: (payload: WorkspaceFileRef) => ReadTextFileResult;
    readWorkspaceBinary: (payload: WorkspaceFileRef & { maxBytes?: number }) => ReadBinaryFileResult;
    resolveAttachment: (payload: ResolveAttachmentPayload) => ResolveAttachmentResult;
    readAttachmentText: (payload: AttachmentFileRef) => ReadAttachmentTextResult;
    readAttachmentBinary: (payload: ReadAttachmentBinaryPayload) => ReadAttachmentBinaryResult;
    openAttachment: (payload: AttachmentSourceRef) => OpenAttachmentResult;
  };
  media: {
    thumbnails: (payload: MediaThumbnailsPayload) => MediaThumbnailResult;
    resolveGatewayUrl: (payload: MediaResolveGatewayUrlPayload) => MediaResolveGatewayUrlResult;
  ...
  sync: {
    status: (payload?: SyncStatusPayload) => SyncStatusResult;
    upload: (payload: SyncRunPayload) => SyncRunResult;
    download: (payload: SyncRunPayload) => SyncRunResult;
  };
  auth: {
    ...
    listDevices: () => AuthDeviceListResult;
    revokeDevice: (payload: AuthRevokeDevicePayload) => HostSuccess;
  };
};
