...
# Post-installation script for GrandPoem Studio on Linux
...
# Create symbolic link for GrandPoem Studio app binary
if [ -x /opt/GrandPoem Studio/grandpoem-studio ]; then
    ln -sf /opt/GrandPoem Studio/grandpoem-studio /usr/local/bin/grandpoem-studio 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/GrandPoem Studio/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/GrandPoem Studio/chrome-sandbox' || true
else
    chmod 0755 '/opt/GrandPoem Studio/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/GrandPoem Studio/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/grandpoem-studio'
...
echo "GrandPoem Studio has been installed successfully."
