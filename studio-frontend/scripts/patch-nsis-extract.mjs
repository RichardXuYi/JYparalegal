...
const PATCH_MARKER = 'GrandPoem Studio-patched-v2: extract directly to $INSTDIR and fail closed';
const LEGACY_PATCH_MARKER = 'GrandPoem Studio-patched: extract directly to $INSTDIR';
const LEGACY_CONTINUE_ON_EXTRACT_FAILURE = 'continuing overwrite install anyway';
const FATAL_EXTRACT_FAILURE_DETAIL = 'Failed to extract GrandPoem Studio files after multiple attempts.';
const ROLLBACK_EXTRACT_FAILURE_DETAIL = 'Restoring previous GrandPoem Studio installation after failed update';

const PATCHED_EXTRACT_MACRO = [
  '!macro extractUsing7za FILE',
  `  ; ${PATCH_MARKER}.`,
  '  StrCpy $R9 0',
  '  grandpoem-studio_extract_attempt:',
  '    IntOp $R9 $R9 + 1',
  '    DetailPrint "Extracting GrandPoem Studio application files (attempt $R9, please wait)..."',
  '    SetOutPath $INSTDIR',
  '    ClearErrors',
  '    Nsis7z::Extract "${FILE}"',
  '    IfErrors 0 grandpoem-studio_extract_done',
  '    ${if} $R9 < 5',
  ...
  '      Pop $1',
  '      Sleep 3000',
  '      Goto grandpoem-studio_extract_attempt',
  '    ${endIf}',
  '    DetailPrint "Failed to extract GrandPoem Studio files after multiple attempts."',
  '    ${if} $grandpoem_studioRollbackDir != ""',
  '      IfFileExists "$grandpoem_studioRollbackDir\\" 0 grandpoem-studio_extract_show_error',
  '      DetailPrint "Restoring previous GrandPoem Studio installation after failed update..."',
  '      SetOutPath $TEMP',
  '      RMDir /r "$INSTDIR"',
  '      Rename "$grandpoem_studioRollbackDir" "$INSTDIR"',
  '    ${endIf}',
  '  grandpoem-studio_extract_show_error:',
  '    MessageBox MB_OK|MB_ICONEXCLAMATION "$(decompressionFailed)" /SD IDOK',
  '    SetErrorLevel 2',
  '    Quit',
  '  grandpoem-studio_extract_done:',
  '!macroend',
].join('\n');
...
export function patchNsisExtractTemplate(targetPath = EXTRACT_APP_PACKAGE_NSH) {
  ...
  if (hasStaleExtractPatch(original)) {
    console.warn('[patch-nsis-extract] Stale GrandPoem Studio extract patch detected; replacing with fail-closed patch.');
