 ...
 * replacement in that inner process, so the old GrandPoem Studio.exe can remain alive,
...
const PATCH_MARKER = 'GrandPoem Studio-patched-v2: run app-running guard in assisted UAC inner instance';
...
const PATCHED_ASSISTED_CHECK = [
  ...
  '  ; UAC inner instance; skipping CHECK_APP_RUNNING there can leave GrandPoem Studio.exe',
