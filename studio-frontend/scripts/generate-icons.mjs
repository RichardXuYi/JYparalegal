...
echo`馃帹 Generating GrandPoem Studio icons using Node.js...`;

// Check if SVG source exists
if (!fs.existsSync(SVG_SOURCE)) {
  echo`鉂?SVG source not found: ${SVG_SOURCE}`;
  process.exit(1);
}
...
try {
  ...
  echo`  鉁?Created icon.png (512x512)`;
  ...
  if (icnsBuffer) {
    ...
    echo`  鉁?Created icon.icns`;
  } else {
    echo(chalk.red`  鉂?Failed to create icon.icns`);
  }

  // 4. Generate Linux PNGs (various sizes)
  echo`馃惂 Generating Linux PNG icons...`;
  ...
  echo`  鉁?Created ${generatedCount} Linux PNG icons`;

  // 5. Generate macOS Tray Icon Template
  echo`馃搷 Generating macOS tray icon template...`;
  ...
  if (fs.existsSync(TRAY_SVG_SOURCE)) {
    ...
    echo`  鉁?Created tray-icon-Template.png (22x22)`;
  } else {
    echo`  鈿狅笍  tray-icon-template.svg not found, skipping tray icon generation`;
  }

  echo`\n鉁?Icon generation complete! Files located in: ${ICONS_DIR}`;

} catch (error) {
  echo(chalk.red`\n鉂?Fatal Error: ${error.message}`);
  process.exit(1);
}
