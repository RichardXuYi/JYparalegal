...
/** Auto-detect a human-readable device name from the OS. */
function detectDeviceName(): string {
  const osName = platform() === 'win32' ? 'Windows' : platform() === 'darwin' ? 'macOS' : 'Linux';
  return `${osName} ${release()} - Studio Desktop`;
}
...
async function deleteJson<T>(path: string, accessToken?: string | null): Promise<{
  status: number;
  envelope: ApiEnvelope<T> | null;
}> {
  const headers: Record<string, string> = {};
...
const PLATFORM_REWRITES: Array<[string, string]> = [
...
function rewritePlatformPath(rawPath: string): string | null {
  const qIndex = rawPath.indexOf('?');
  const pathname = qIndex >= 0 ? rawPath.slice(0, qIndex) : rawPath;
  const qs = qIndex >= 0 ? rawPath.slice(qIndex) : '';
  for (const [from, to] of PLATFORM_REWRITES) {
    if (pathname === from || pathname.startsWith(`${from}/`)) {
      return `${to}${pathname.slice(from.length)}${qs}`;
    }
  }
  return null;
}

/** Forward a studio `/platform/**` call to the Java API with the stored bearer token. */
export async function proxyPlatformRequest(input: {
  path: string;
  method?: string;
  body?: string | null;
  ...
  if (!input.path.startsWith('/platform')) {
    return { status: 404, body: JSON.stringify({ code: 404, msg: 'not a platform path', data: null }) };
  }
  const backendPath = rewritePlatformPath(input.path);
  if (!backendPath) {
    return { status: 404, body: JSON.stringify({ code: 404, msg: 'unknown platform route', data: null }) };
  }

  const method = (input.method || 'GET').toUpperCase();
  const hasBody = method === 'POST' || method === 'PUT' || method === 'PATCH';
  const { accessToken } = await readAuth();
  if (!accessToken) return { status: 401, body: unauthenticated };
  ...
  if (result.status === 401) {
    const refreshed = await tryRefresh().catch(() => null);
    if (!refreshed) return { status: 401, body: unauthenticated };
 ...
 *
 * Restarts are serialized on a chain: a rapid logout→login sequence must not
 * lose the second restart (governor cooldown suppression / restartInFlight
 * join), otherwise the Gateway keeps running on the previous user's scope and
 * accounts start "fighting over" one OpenClaw directory.
 */
let scopeChangeEpoch = 0;
let scopeRestartChain: Promise<void> = Promise.resolve();

export function applyScopeChange(ctx: AuthApiContext | undefined, userId: number | null): void {
  const scopeChanged = setActiveScopeUser(userId);
  if (!scopeChanged || !ctx) return;

  const gatewayManager = ctx.gatewayManager;
  const epoch = ++scopeChangeEpoch;
    ...
    .then(async () => {
      const restartForEpoch = async (): Promise<void> => {
        ...
        try {
          ...
          console.warn('[auth] Gateway restart after account scope change failed:', error);
        }
      };

      // Superseded before we even started: skip straight to the convergence
      // pass so only the latest scope gets restarted.
      if (epoch === scopeChangeEpoch) {
        await restartForEpoch();
      }
      // Convergence pass: a newer scope change that arrived while our
      // restart was running still needs its own restart (ours may have been
      // joined into an older in-flight restart or deferred).
      if (epoch !== scopeChangeEpoch) {
        await restartForEpoch();
      }
    })
...
/** Test seam: resolves once all queued scope-change restarts have settled. */
export function waitForScopeRestartChainForTest(): Promise<void> {
  return scopeRestartChain;
}
...
export function createAuthApi(ctx?: AuthApiContext): CompleteHostServiceRegistry['auth'] {
  return {
    login: async (payload): Promise<AuthLoginResult> => {
      ...
      try {
        ({ status, envelope } = await postJson<LoginData>('/api/auth/login', { username, password, deviceName: detectDeviceName() }));
      } catch (error) {
    ...
    listDevices: async (): Promise<AuthDeviceListResult> => {
      ...
      const { status, envelope } = await getJson<AuthDeviceListResult>('/api/auth/devices', accessToken);
      if (status === 401) {
        const refreshed = await tryRefresh().catch(() => null);
        if (refreshed) {
          const retry = await getJson<AuthDeviceListResult>('/api/auth/devices', refreshed);
          if (isOk(retry.status, retry.envelope) && Array.isArray(retry.envelope?.data)) {
            return { success: true, devices: retry.envelope.data as AuthDeviceListResult['devices'] };
          }
        }
        return { success: false, error: 'not_authenticated' };
      }
      // Backend returns the device list directly in `data` (an array).
      if (!isOk(status, envelope) || !Array.isArray(envelope?.data)) {
    ...
    revokeDevice: async (payload): Promise<{ success: boolean; error?: string }> => {
      const { accessToken } = await readAuth();
      if (!accessToken) {
        return { success: false, error: 'not_authenticated' };
      }
      const deviceId = (payload as AuthRevokeDevicePayload)?.deviceId;
      if (typeof deviceId !== 'number') {
        return { success: false, error: 'invalid deviceId' };
      }
      const { status, envelope } = await deleteJson(`/api/auth/devices/${deviceId}`, accessToken);
      if (status === 401) {
        const refreshed = await tryRefresh().catch(() => null);
        if (refreshed) {
          const retry = await deleteJson(`/api/auth/devices/${deviceId}`, refreshed);
          if (isOk(retry.status, retry.envelope)) return { success: true };
        }
        return { success: false, error: 'not_authenticated' };
      }
