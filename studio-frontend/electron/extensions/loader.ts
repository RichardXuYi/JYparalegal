...
/**
 * Allowlist of external npm package names that may be loaded via require().
 * Builtin extensions (registered via registerBuiltinExtension) bypass this
 * check because they are instantiated directly, not through require().
 */
...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'grandpoem-extensions.json');
  }
  return join(app.getAppPath(), 'grandpoem-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No grandpoem-extensions.json found, loading all builtin extensions');
  }
  ...
  for (const extensionId of mainExtensions) {
    ...
    try {
      if (!ALLOWED_EXTERNAL_EXTENSIONS.has(extensionId)) {
        throw new Error(`Extension not in allowlist: ${extensionId}`);
      }
