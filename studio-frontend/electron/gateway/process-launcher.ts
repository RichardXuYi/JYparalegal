...
(function () {
  ...
  globalThis.fetch = function grandpoemStudioFetch(input, init) {
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      flat['X-OpenRouter-Title'] = 'GrandPoem Studio';
      init.headers = flat;
    }
...
export async function launchGatewayProcess(options: {
  ...
  // which causes:
  //   - cross-machine name collisions when multiple OpenClaw/GrandPoem Studio peers
  //     share a LAN (each falls back to "<name> (OpenClaw) (2)")
  //   - self-collisions on multi-homed hosts (Wi-Fi + Tailscale + utun ...)
  //   - "ghost" record collisions after an unclean GrandPoem Studio exit, because
  //     SIGKILL prevents ciao from emitting the mDNS goodbye record.
  //
  // GrandPoem Studio has no UI for LAN gateway discovery today, so the advertiser is
