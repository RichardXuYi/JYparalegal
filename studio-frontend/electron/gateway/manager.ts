...
export class GatewayManager extends EventEmitter {
  ...
  private async initDeviceIdentity(): Promise<void> {
    ...
    try {
      const identityPath = path.join(app.getPath('userData'), 'grandpoem-studio-device-identity.json');
  ...
  async start(): Promise<void> {
    ...
    this.startLock = true;
    // Fleet mode assigns a per-user port (CLAWX_PORT_OPENCLAW_GATEWAY); the
    // state controller historically initialized with a hardcoded port, so
    // re-assert the dynamic port here to prevent it from being overridden.
    this.setStatus({ port: getPort('OPENCLAW_GATEWAY') });
  ...
  async restart(): Promise<void> {
    ...
    if (!decision.allow) {
      ...
      // Never silently drop a requested restart: a rapid account switch
      // (logout→login) can otherwise leave the Gateway running on the wrong
      // user scope. Wait out the cooldown and retry once — the deferral and
      // in-flight guards re-run, and a concurrent restart may already have
      // covered this request.
      await new Promise((resolve) => setTimeout(resolve, decision.retryAfterMs + 100));
      await this.restart();
      return;
    }
  ...
  private async startProcess(): Promise<void> {
    // Defensive: never spawn on anything but the supervisor-assigned port.
    const expectedPort = getPort('OPENCLAW_GATEWAY');
    if (this.status.port !== expectedPort) {
      logger.warn(`Gateway port drift detected: ${this.status.port} != ${expectedPort}; using dynamic port`);
      this.setStatus({ port: expectedPort });
    }
   ...
   * @param _externalToken - Reserved for future use (external token authentication)
   */
