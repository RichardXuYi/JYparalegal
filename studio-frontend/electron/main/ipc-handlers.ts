...
import { getOpenClawStatus, getOpenClawSkillsDir, getOpenClawConfigDir, ensureDir, expandPath } from '../utils/paths';
...
/** Cron repair interval ID for cleanup */
let cronRepairIntervalId: NodeJS.Timeout | null = null;

/**
 * Handle proxy settings change - shared by multiple IPC handlers
 */
...
/**
 * Cleanup IPC handlers (called during app shutdown)
 */
export function cleanupIpcHandlers(): void {
  if (cronRepairIntervalId) {
    clearInterval(cronRepairIntervalId);
    cronRepairIntervalId = null;
  }
}
          ...
          if (request.action === 'save') {
            ...
            try {
              ...
              try {
                ...
                logger.warn('Failed to sync openclaw provider config:', err);
              }
            ...
            try {
              ...
              if (existing?.type) {
                try {
                  ...
                  logger.warn('Failed to completely remove provider from OpenClaw:', err);
                }
              }
              data = { success: true };
            } catch (error) {
          ...
          if (request.action === 'updateWithKey') {
            ...
            try {
              ...
              try {
                ...
                logger.warn('Failed to sync openclaw config after provider update:', err);
              }
...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  cronRepairIntervalId = setInterval(async () => {
    try {
      ...
      for (const job of jobs) {
        ...
        if (
          ...
          if (correctAgentId) {
            logger.debug(`Periodic repair: job "${job.name}" agentId undefined -> "${correctAgentId}"`);
      ...
      if (now - _lastRepairErrorLogAt >= REPAIR_ERROR_LOG_INTERVAL_MS) {
        ...
        logger.debug('Periodic cron repair error:', error);
      }
    }
...
function registerGatewayHandlers(gatewayManager: GatewayManager): void {
  ...
  // Gateway RPC is handled via host:invoke (gateway.rpc action) in gateway-api.ts.
  // The direct gateway:rpc ipcMain.handle was dead code — preload's validChannels
  // whitelist never included it, so the renderer could never invoke it.
...
function registerProviderHandlers(gatewayManager: GatewayManager): void {
  ...
  // Get the API key in masked form to avoid leaking plaintext secrets to the
  // renderer. Only the last 4 characters are shown; the host API path
  // (providers.getApiKey) handles legitimate internal key access.
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        logger.debug(`[grandpoem-studio-validate] validating provider type: ${providerType}`);
...
// 鈹€鈹€ Mime type helpers 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€ File preview (sandboxed) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
//
