...
export async function saveWindowState(win: BrowserWindow): Promise<void> {
  ...
  // Capture bounds synchronously before any await to avoid race conditions
  // where the window is destroyed during the async store operation (e.g. on
  // 'close' during quit the window may be torn down before the promise settles).
