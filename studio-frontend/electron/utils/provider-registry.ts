...
/**
 * Strip a plan-account suffix (e.g. "-124880ac") from a provider key to recover
 * the base vendor id. Returns the original key when it does not match the
 * plan-account pattern `${vendorId}-[8hex]`.
 */
function stripPlanAccountSuffix(type: string): string {
  const match = type.match(/^(.+)-[0-9a-f]{8}$/);
  return match ? match[1] : type;
}

/** Get the environment variable name for a provider type */
export function getProviderEnvVar(type: string): string | undefined {
  return getSharedProviderEnvVar(type)
    ?? getSharedProviderEnvVar(stripPlanAccountSuffix(type))
    ?? EXTRA_ENV_ONLY_PROVIDERS[type]?.envVar;
}
...
export function getProviderDefaultModel(type: string): string | undefined {
  return getSharedProviderDefaultModel(type) ?? getSharedProviderDefaultModel(stripPlanAccountSuffix(type));
}
...
export function getProviderConfig(
  ...
  return (getProviderBackendConfig(type) ?? getProviderBackendConfig(stripPlanAccountSuffix(type))) as ProviderBackendConfig | undefined;
}
