...
export function buildDefaultGrandPoemStudioIdentityContent(): string {
  return [
    '# IDENTITY.md - GrandPoem Studio',
    '',
    '- **Name:** GrandPoem Studio',
    '- **Creature:** desktop AI assistant',
    '- **Vibe:** concise, capable, and practical',
    '- **Emoji:** 馃惥',
    '- **Avatar:**',
    '',
    'GrandPoem Studio uses a default desktop identity instead of chat-first bootstrap.',
    '',
  ].join('\n');
}
...
/**
 * Ensure GrandPoem Studio-managed workspaces have a non-template IDENTITY.md before the
...
export async function ensureGrandPoemStudioIdentityFile(
  ...
  if (await fileExists(bootstrapPath)) {
    try {
      ...
      logger.info(`Removed chat-first bootstrap file from GrandPoem Studio workspace (${resolvedWorkspaceDir})`);
    } catch {
      logger.warn(`Failed to remove chat-first bootstrap file: ${bootstrapPath}`);
    }
  } else if (wroteIdentity) {
    logger.info(`Seeded default GrandPoem Studio identity for workspace (${resolvedWorkspaceDir})`);
  }
}
...
/**
 * Merge a GrandPoem Studio context section into an existing file's content.
 ...
 * for GrandPoem Studio-managed workspaces.  Removes everything from the heading
...
/**
 * Detect and remove bootstrap .md files that contain only GrandPoem Studio markers
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed GrandPoem Studio-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove GrandPoem Studio-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge GrandPoem Studio context snippets into workspace bootstrap files that already
...
async function mergeGrandPoemStudioContextOnce(options: EnsureGrandPoemStudioContextOptions = {}): Promise<MergeResult> {
  ...
  if (!(await fileExists(contextDir))) {
    logger.debug('GrandPoem Studio context directory not found, skipping context merge');
  ...
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    ...
    for (const file of files) {
      ...
      // First Run stripping happened and the GrandPoem Studio section stayed identical.
      if (merged !== originalExisting) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged GrandPoem Studio context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
/**
 * Ensure GrandPoem Studio context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
...
async function runEnsureGrandPoemStudioContext(options: EnsureGrandPoemStudioContextOptions): Promise<void> {
  ...
  if (result.retryableMissing === 0) {
    if (result.missing > 0) {
      logger.debug(`GrandPoem Studio context merge skipped ${result.missing} non-ready file(s)`);
    }
    return;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (result.retryableMissing === 0) {
      logger.info(`GrandPoem Studio context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`GrandPoem Studio context merge: ${result.retryableMissing} startup file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`GrandPoem Studio context merge: ${result.retryableMissing} startup file(s) still missing after ${MAX_RETRIES} retries`);
}
