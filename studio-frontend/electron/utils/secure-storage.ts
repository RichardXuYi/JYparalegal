...
import { getGrandPoemStudioProviderStore } from '../services/providers/store-instance';
import {
...
export async function storeApiKey(providerId: string, apiKey: string): Promise<boolean> {
  try {
    ...
    const s = await getGrandPoemStudioProviderStore();
 ...
 * Also synchronizes GrandPoem Studio local provider list with OpenClaw's actual config.
 */
...
> {
  ...
  for (const provider of providers) {
    ...
    // we should remove it from GrandPoem Studio UI to stay consistent.
    ...
    // e.g. provider.id "custom-a1b2c3d4-..." 鈫?strip hyphens 鈫?"customa1b2c3d4..." 鈫?slice(0,8) 鈫?"customa1"
    // 鈫?openClawKey = "custom-customa1"
    ...
    if (configMissing || (!isBuiltin && !isActive)) {
      logger.debug(`[Sync] Provider ${provider.id} (${provider.type}) missing from OpenClaw, hiding from UI`);
