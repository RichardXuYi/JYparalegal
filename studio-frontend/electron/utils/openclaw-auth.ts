 ...
 *
 * This intentionally reads auth-profiles.json rather than GrandPoem Studio's provider
...
/**
 * Backfill `reserveTokensFloor` on compaction configs that GrandPoem Studio or OpenClaw
 ...
 * `contextTokens`. Rows written by older GrandPoem Studio versions only carried
 ...
 * GrandPoem Studio-written entries historically only included `{ id, name }`.
 *
 ...
 * The bundled OpenClaw distribution GrandPoem Studio ships does not register that harness,
 ...
 * (written by earlier GrandPoem Studio builds that did not pin the runtime) get
 * repaired before the next Gateway reload picks them up. Without this, users
 * who upgrade GrandPoem Studio while still pointing at an OpenAI provider would keep
...
/**
 * Configure a GrandPoem Studio-owned OpenAI-compatible image provider.
...
export function readOpenAiCompatibleImageRelayState(
  ...
  // Backward compatibility for GrandPoem Studio builds that used models.providers.openai
  // for image relay. New saves move to the GrandPoem Studio-owned provider above.
 ...
 * Used by GrandPoem Studio to seed the provider store when it's empty but providers are
...
/**
 * Write the GrandPoem Studio gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    const gatewayPort = (await getSetting('gatewayPort')) || getPort('OPENCLAW_GATEWAY');
 ...
 * which means conversations disappear after roughly one day.  GrandPoem Studio sets
 ...
 * OpenClaw/GrandPoem Studio versions or external tools.
 *
...
// GrandPoem Studio desktop does not expose the subagent workflow. Deny the spawn entry
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // for GrandPoem Studio to properly integrate with its updated tool system.
    ...
    // GrandPoem Studio keeps direct skill-creator authoring instead, so deny the workshop
    ...
    if (denyResult.modified) {
      ...
      toolsModified = true;
      console.log('[sanitize] Added "skill_workshop" to tools.deny for GrandPoem Studio desktop');
    } else if (!Array.isArray(toolsConfig.deny) || toolsConfig.deny.length !== denyResult.deny.length) {
      toolsConfig.deny = denyResult.deny;
      toolsModified = true;
    }

    // GrandPoem Studio desktop does not expose the subagent workflow. Deny the spawn
    ...
    if (subagentDenyResult.modified) {
      ...
      toolsModified = true;
      console.log(`[sanitize] Added subagent tool(s) to tools.deny for GrandPoem Studio desktop: ${SUBAGENT_TOOL_DENY_ENTRIES.join(', ')}`);
    ...
    // GrandPoem Studio is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for GrandPoem Studio desktop');
    }
    ...
    // agent:main:main), which makes GrandPoem Studio sidebar conflate feishu, dingtalk,
    ...
    if (sessionConfig.dmScope !== 'per-channel-peer' && sessionConfig.dmScope !== 'per-account-channel-peer') {
      ...
      modified = true;
      console.log('[sanitize] Set session.dmScope="per-channel-peer" so channel DMs appear as separate sessions in GrandPoem Studio');
    }
    ...
    if (gatewayDenyResult.modified) {
      ...
      console.log('[sanitize] Added "skill_workshop" to gateway.tools.deny for GrandPoem Studio desktop');
    ...
    if (gatewaySubagentDenyResult.modified) {
      ...
      gatewayModified = true;
      console.log(`[sanitize] Added subagent tool(s) to gateway.tools.deny for GrandPoem Studio desktop: ${SUBAGENT_TOOL_DENY_ENTRIES.join(', ')}`);
    ...
    if (autonomous.enabled !== false) {
      ...
      skillsModified = true;
      console.log('[sanitize] Disabled skills.workshop.autonomous for GrandPoem Studio desktop');
    }
    ...
    if (skillCreatorEntry.enabled !== true) {
      ...
      skillsModified = true;
      console.log('[sanitize] Enabled bundled skill-creator for direct skill authoring in GrandPoem Studio desktop');
    }
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // GrandPoem Studio bundles Feishu via the official @larksuite/openclaw-lark
      ...
      // GrandPoem Studio-critical bundled plugins, active provider plugins, and explicitly
