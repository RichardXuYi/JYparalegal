 ...
 * Only known setting keys (defined in AppSettings) are accepted; unknown
 * keys are silently dropped to prevent store pollution or override of
 * internal fields.
 */
export async function importSettings(json: string): Promise<void> {
  try {
    ...
    const allowedKeys = new Set<keyof AppSettings>([
      ...
      'telemetryEnabled', 'machineId', 'hasReportedInstall',
      'gatewayAutoStart', 'gatewayPort', 'gatewayToken',
      ...
      'autoSyncSkills', '__migratedAutoSyncSkills',
      'sidebarCollapsed', 'devModeUnlocked',
      'selectedBundles', 'enabledSkills', 'disabledSkills',
    ]);
    const filtered: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(settings)) {
      if (allowedKeys.has(key as keyof AppSettings)) filtered[key] = value;
    }
    store.set(filtered);
  } catch {
