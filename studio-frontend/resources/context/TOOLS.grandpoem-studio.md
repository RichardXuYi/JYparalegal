## GrandPoem Studio Tool Notes

### uv (Python)

- `uv` is bundled with GrandPoem Studio and on PATH. Do NOT use bare `python` or `pip`.
- Run scripts: `uv run python <script>` | Install packages: `uv pip install <package>`

### Browser

- `browser` tool provides full automation (scraping, form filling, testing) via an isolated managed browser.
- Flow: `action="start"` 鈫?`action="snapshot"` (see page + get element refs like `e12`) 鈫?`action="act"` (click/type using refs).
