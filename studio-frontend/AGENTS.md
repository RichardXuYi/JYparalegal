...
GrandPoem Studio is a cross-platform **Electron desktop app** (React 19 + Vite + TypeScript) providing a GUI for the OpenClaw AI agent runtime. It uses pnpm as its package manager (pinned version in `package.json`'s `packageManager` field).
...
- **Gateway startup**: When running `pnpm dev`, the OpenClaw Gateway process starts automatically on port 18789. It takes ~10-30 seconds to become ready. Gateway readiness is not required for UI development鈥攖he app functions without it (shows "connecting" state).
...
- **UI change validation**: Any user-visible UI change should pass `pnpm run typecheck` and `pnpm run lint:check` in the same PR. Playwright E2E was removed in v1.4.60 (see root `docs/test-cleanup-list.md`).
- **i18n & styling conventions**: New user-facing features must (1) route all text through `react-i18next` with full locale coverage (`de` / `en` / `fr` / `zh` under `shared/i18n/locales/<lang>/<ns>.json`) 鈥?never hardcode display strings, and (2) use the design tokens and substitution rules documented in `src/styles/globals.css` (surfaces `bg-surface-modal` / `bg-surface-input`, selected state `bg-black/5 dark:bg-white/10`, status colours `text-X-700 dark:text-X-400`, page H1/H2 `font-serif font-normal tracking-tight`, etc.) 鈥?see the *Component conventions* block in `globals.css` for the full substitution table.
...
- **Harness CI/local parity**: The `harness:ci` script was removed in v1.4.60 (it depended on deleted test files); run `pnpm harness validate --spec <task-spec>` / `pnpm harness run --spec <task-spec>` locally instead. Real task specs should be validated without `--no-diff`; `--no-diff` is only for structural checks of checked-in examples.
