...
Gateway backend communication covers all GrandPoem Studio paths that move data between the visual desktop UI and OpenClaw runtime/backend services.
...
Channel/plugin migration behavior is also part of this scenario when GrandPoem Studio rewrites OpenClaw config before Gateway launch. Upgrades must preserve single-owner channel registration for migrated plugin-backed channels such as Feishu/Lark.
