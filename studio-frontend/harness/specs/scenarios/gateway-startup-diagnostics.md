...
Use this spec when GrandPoem Studio shows the Gateway as starting/running but UI data does not refresh, Dreams cannot load, or Gateway RPC calls time out after a restart.

GrandPoem Studio should prefer OpenClaw-native signals over stderr string matching:
...
- `gateway.ready`, `health`, and `presence` events should update GrandPoem Studio's main-process capability cache.
...
- **Handshake ready** only means GrandPoem Studio connected to the Gateway socket.
...
2. Read recent GrandPoem Studio logs:

```bash
tail -n 160 "$HOME/Library/Application Support/grandpoem-studio/logs/grandpoem-studio-$(date +%F).log"
```
...
```bash
pnpm exec openclaw gateway call system-presence >/tmp/grandpoem-studio-system-presence.json
pnpm exec openclaw gateway call health --params '{"probe":false}' >/tmp/grandpoem-studio-health.json
pnpm exec openclaw gateway call status >/tmp/grandpoem-studio-status.json
pnpm exec openclaw gateway call channels.status --params '{"probe":false}' >/tmp/grandpoem-studio-channels-status.json
pnpm exec openclaw gateway call doctor.memory.status >/tmp/grandpoem-studio-memory-status.json
pnpm exec openclaw gateway call doctor.memory.dreamDiary >/tmp/grandpoem-studio-dream-diary.json
```
...
```bash
sample <gateway-pid> 3 -mayDie >/tmp/grandpoem-studio-gateway.sample.txt
```
...
- Where the artifact will be written, usually `/tmp/grandpoem-studio-gateway.sample.txt`.
- What will be inspected and what will not be shared verbatim.

Do not proceed without explicit user agreement when:

- Sampling a process that is not clearly the GrandPoem Studio-owned Gateway child.
...
- Whether the finding points to GrandPoem Studio-owned prelaunch cleanup, OpenClaw runtime startup cost, active user work, or inconclusive data.
...
- Logs no longer repeat stale runtime cache or escaped managed-skill symlink warnings for entries GrandPoem Studio can safely clean.
...
- Separate GrandPoem Studio-owned cleanup issues from OpenClaw runtime initialization cost.
