...
- Discover: GrandPoem Studio scans bundled and external sources and produces a factual inventory. Discovery does not choose active owners.
- Resolve: GrandPoem Studio chooses canonical package/capability owners, applies migration rules, and marks conflicts before config activation.
- Materialize: GrandPoem Studio installs, upgrades, copies, links, or verifies the physical package selected by resolution.
- Validate: GrandPoem Studio verifies package manifests, dependencies, capability config, ownership uniqueness, and startup requirements.
