...
- Every new `GatewayStatus` funnel in the renderer goes through `applyGatewayStatus` in `src/stores/gateway.ts` (records `hasSeenRunningThisSession`, syncs `src/stores/gateway-ui.ts`). No component may dim or block a rendered page while connecting: boot waiting is carried by the full-screen `InitializingScreen` (`shouldHoldBootScreen`, capped by `BOOT_SCREEN_CAP_MS`), and runtime dips use the non-blocking banner only.
