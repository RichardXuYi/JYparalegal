...
expectedUserBehavior:
  ...
  - OpenClaw's trajectory artefacts for the same session id are removed too 鈥?both the local <id>.trajectory.jsonl flight recorder and the <id>.trajectory-path.json pointer sidecar.
...
Conversation deletion in GrandPoem Studio runs entirely on the Main process 鈥?the
