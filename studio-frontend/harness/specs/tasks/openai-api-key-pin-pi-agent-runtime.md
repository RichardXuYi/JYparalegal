...
expectedUserBehavior:
  ...
  - Upgrading from an earlier GrandPoem Studio build that wrote `openai` or `openai-codex` provider entries without `agentRuntime` self-heals those entries on the next provider switch and during pre-launch config sanitization before Gateway reads the config.
  - A pre-existing user-supplied non-empty `agentRuntime.id` is preserved; the GrandPoem Studio pin only fills missing runtime policy.
requiredProfiles:
  - fast
  - comms
requiredRules:
...
requiredTests:
  - tests/unit/openclaw-auth.test.ts
acceptance:
  - `electron/utils/openclaw-auth.ts` applies a shared OpenAI runtime-pin helper from all GrandPoem Studio write/self-heal paths and pins `agentRuntime: { id: 'pi' }` on `models.providers.openai` and `models.providers.openai-codex` entries that lack a non-empty runtime id.
...
trajectory, but the shipped GrandPoem Studio distribution does not register an agent
...
the failure is about agent harness selection. GrandPoem Studio therefore writes an
