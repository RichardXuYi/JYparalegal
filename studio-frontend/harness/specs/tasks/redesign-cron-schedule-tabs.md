...
expectedUserBehavior:
  ...
  - One-time ("at") tasks are auto-cleared by the OpenClaw runtime after they run (OpenClaw defaults `deleteAfterRun` to true for `at` schedules); GrandPoem Studio does not try to keep them as paused records.
...
One-time tasks require a Gateway `{ kind: 'at', at }` schedule. The GrandPoem Studio cron host
