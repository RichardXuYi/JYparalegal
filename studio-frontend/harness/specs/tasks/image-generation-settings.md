...
touchedAreas:
  ...
  - resources/openclaw-plugins/grandpoem-studio-openai-image/index.mjs
  - resources/openclaw-plugins/grandpoem-studio-openai-image/openclaw.plugin.json
  - resources/openclaw-plugins/grandpoem-studio-openai-image/package.json
...
expectedUserBehavior:
  ...
  - Saving the OpenAI-compatible image endpoint writes a GrandPoem Studio-owned provider (`grandpoem-studio-openai-image`) and auth profile, enables `request.allowPrivateNetwork` for trusted custom endpoints, and leaves `models.providers.openai` untouched so chat continues to use the regular OpenAI provider.
...
`~/.openclaw/agents/{id}/agent/auth-profiles.json`. GrandPoem Studio's OpenAI-compatible
image endpoint uses a separate `grandpoem-studio-openai-image` provider/plugin so image
base URL and API key can differ from the normal `openai` chat provider.

GrandPoem Studio syncs chat defaults on provider switch, but image generation is configured independently from its developer-only Image Generation page and is never auto-synced from the default chat provider.
