...
import { isGrandPoemStudioDesktopSessionKey, shouldIncludeSessionInSidebarList } from './session-key-utils';
import { pickStartupSessionFallback, resolveStartupSessionKey } from './session-selection';
import { clearLastSessionKey, persistLastSessionKey, readLastSessionKey } from './last-session';
...
// Startup session resolution must run exactly once per app load: the first
// sessions.list response validates the persisted last-session key (or falls
// back to the most recently active session). Any explicit user navigation
// (switch/new session) before that also counts as resolved.
let hasResolvedStartupSession = false;
    ...
    loadSessions: async () => {
      try {
        ...
        if (data) {
          ...
          if (!hasResolvedStartupSession) {
            hasResolvedStartupSession = true;
            // Base the decision on the persisted last-session key (not the
            // store default) so fresh installs land on the most recently
            // active session instead of the hard-coded main-agent session.
