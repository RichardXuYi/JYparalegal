...
import type { SkillsStatusResult, SkillSyncResult } from '@/lib/host-api';
...
interface SkillsState {
  ...
  lastSyncSummary: { pushed: number; pulled: number; failed: number } | null;
  ...
  /** Drop all skill state when the account changes. */
...
export const useSkillsStore = create<SkillsState>((set, get) => ({
  ...
  syncSkills: async () => {
    ...
    try {
      const result = await hostApi.skills.sync();
      if (!result.success) {
        set({ syncError: result.error || 'Sync failed' });
        return result;
      }
      // Pulled skills may have landed on disk — refresh the local list.
      ...
      const appError = normalizeAppError(error, { module: 'skills', operation: 'fetch' });
      set({ syncError: appError.message });
      return { success: false, error: appError.message } satisfies SkillSyncResult;
    } finally {
      set({ syncing: false });
    }
  },
