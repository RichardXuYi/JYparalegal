...
import { isGrandPoemStudioDesktopSessionKey, shouldIncludeSessionInSidebarList } from './chat/session-key-utils';
import { fetchCronSessionHistory } from '@/lib/cron-session-history';
import { pickStartupSessionFallback, resolveStartupSessionKey } from './chat/session-selection';
import { clearLastSessionKey, persistLastSessionKey, readLastSessionKey } from './chat/last-session';
import {
...
import {
  clearDeletedSessionKeys,
  persistDeletedSessionKeys,
  readDeletedSessionKeys,
} from './chat/deleted-sessions';
...
// Bumped by resetForUserSwitch: async loads (sessions/history) that were
// started for the previous account must not write into the next account's
// state, even when the session key happens to be identical (default key).
...
// First sessions.list after launch may retarget the current session to the
// persisted last-used (or most recent) session; explicit user navigation
// (switch/new session) before that permanently wins.
let _hasResolvedStartupSession = false;
...
// Sessions hard-deleted by this browser profile. The Gateway keeps deleted
// sessions in memory and re-serves their history / re-persists their
// sessions.json entry, so loads targeting these keys must be suppressed
// until the user legitimately recreates the session by sending a new message.
// Hydrated from localStorage so the suppression survives page reloads.
const _deletedSessionKeys = readDeletedSessionKeys();
...
const IMAGE_CACHE_KEY = 'grandpoem-studio:image-cache';
...
function trimPathTerminators(filePath: string): string {
  return filePath.replace(/[锛屻€傦紱;,.!?]+$/u, '');
}
...
function extractRawFilePaths(text: string): Array<{ filePath: string; mimeType: string }> {
  ...
  // that macOS' default screenshot filename ("鎴睆 2026-05-06 17.46.51.png")
  ...
  const skillPathBoundary = '(?=$|\\s|[\\x5b\\x5d"\'`(),<>锛屻€傦紱;,.!?])';
...
async function loadMissingPreviews(messages: RawMessage[]): Promise<boolean> {
  // See helpers.ts loadMissingPreviews for the canonical comment block —  // this monolithic copy is kept in sync so legacy chat.ts callers also
...
function buildSessionSwitchPatch(
  ...
  return {
    ...
    thinkingLevels: _thinkingLevelsCache.get(nextSessionKey) ?? [],
    ...cachedRunState,
    error: null,
    runError: null,
  };
...
// 鈹€鈹€ Write tool_use baseline capture 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
//
...
// 鈹€鈹€ Store 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

// Boot on the persisted last-used session instead of the hard-coded
// main-agent session; loadSessions re-validates it against sessions.list.
const _initialSessionKey = readLastSessionKey() ?? DEFAULT_SESSION_KEY;

type SessionThinkingLevelEntry = { id: string; label?: string };

/**
 * Parse the thinking levels supported by a session's model from the gateway
 * session row. Prefers `thinkingLevels` ({id,label} objects); falls back to
 * the legacy `thinkingOptions` string list. Empty array when the gateway
 * reports neither.
 */
function parseSessionThinkingLevels(row: Record<string, unknown>): SessionThinkingLevelEntry[] {
  const levels = row.thinkingLevels;
  ...
  if (Array.isArray(options)) {
    return options
      ...
      .map((entry) => ({ id: entry.trim() }));
  }
  return [];
}

/** Per-session thinking level support, populated by loadSessions and consumed
 * by buildSessionSwitchPatch so switching sessions restores the right options. */
const _thinkingLevelsCache = new Map<string, SessionThinkingLevelEntry[]>();
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sessions: [],
  currentSessionKey: _initialSessionKey,
  currentAgentId: getAgentIdFromSessionKey(_initialSessionKey),
  sessionLabels: {},
  sessionLastActivity: {},
  ...
  // ── Session thinking level override (gateway sessions.patch) ──

  setThinkingLevel: async (level: string | null) => {
    const previous = get().thinkingLevel;
    set({ thinkingLevel: level });
    try {
      // Gateway sessions.patch keys the session row by `key` (mirrors the
      // official Control UI thinking selector); null clears the override.
      await hostApi.gateway.rpc('sessions.patch', {
        key: get().currentSessionKey,
        thinkingLevel: level,
      });
    } catch (error) {
      set({ thinkingLevel: previous });
      throw error;
    }
  },
  ...
  loadSessions: async (force = false, reportError = false) => {
    ...
    _loadSessionsInFlight = (async () => {
      const loadResetGeneration = _resetGeneration;
      try {
        const data = await fetchChatSessionsList();
        // Account switched while this load was in flight: drop the result.
        if (_resetGeneration !== loadResetGeneration) return;
        if (data) {
          const rawSessions = Array.isArray(data.sessions) ? data.sessions : [];
          const thinkingLevelsBySession = new Map<string, SessionThinkingLevelEntry[]>();
          const sessions: ChatSession[] = rawSessions.map((s: Record<string, unknown>) => {
            const key = String(s.key || '');
            if (key) {
              thinkingLevelsBySession.set(key, parseSessionThinkingLevels(s));
            }
            return {
              key,
            ...
            .filter((s: ChatSession) => !_deletedSessionKeys.has(s.key));
          ...
          // First load after launch: restore the persisted last-used session,
          // or fall back to the most recently active regular session. Explicit
          // navigation (switchSession/newSession) before this point wins.
          if (!_hasResolvedStartupSession) {
            _hasResolvedStartupSession = true;
            const startupKey = resolveStartupSessionKey(readLastSessionKey(), dedupedSessions);
            if (startupKey) {
              nextSessionKey = startupKey;
            }
          }
          ...
          const sessionsWithCurrent = !dedupedSessions.find((s) => s.key === nextSessionKey) && nextSessionKey
            && !_deletedSessionKeys.has(nextSessionKey)
            && isGrandPoemStudioDesktopSessionKey(nextSessionKey)
            ? [
              ...dedupedSessions,
          ...
          const nextThinkingLevels = thinkingLevelsBySession.get(nextSessionKey) ?? [];
          for (const [sessionKey, levels] of thinkingLevelsBySession) {
            _thinkingLevelsCache.set(sessionKey, levels);
          }
          if (previousSessionKey !== nextSessionKey) {
            ...
            set((state) => ({
              ...
              thinkingLevels: nextThinkingLevels,
              sessionLastActivity: {
                ...state.sessionLastActivity,
                ...discoveredActivity,
              },
          ...
          persistLastSessionKey(nextSessionKey);
  ...
  // 鈹€鈹€ Switch session 鈹€鈹€

  switchSession: (key: string) => {
    if (key === get().currentSessionKey) return;
    // Explicit navigation supersedes any pending startup restoration.
    _hasResolvedStartupSession = true;
    persistLastSessionKey(key);
  ...
  deleteSession: async (key: string) => {
    ...
    // If the backend deletion failed, reload sessions to restore consistent state
    // instead of leaving the UI out of sync with the on-disk data.
    if (!deleteSuccess) {
    ...
    _deletedSessionKeys.add(key);
    persistDeletedSessionKeys(_deletedSessionKeys);
    ...
    if (currentSessionKey === key) {
      ...
      if (next) {
        persistLastSessionKey(next.key);
      } else {
        clearLastSessionKey();
      }
      set((s) => ({
      ...
      // Deleting the persisted last-used session (while viewing another one)
      // must not leave a stale restore target behind.
      if (readLastSessionKey() === key) {
        clearLastSessionKey();
      }
      set((s) => ({
        sessions: remaining,
    ...
    // Force-refresh the sessions list to sync with the Gateway's updated state.
    // Reset the debounce timer so the call is not throttled.
    _lastLoadSessionsAt = 0;
    await get().loadSessions();
  },
  ...
  // 鈹€鈹€ Cleanup empty session on navigate away 鈹€鈹€
  ...
  // 鈹€鈹€ Load chat history 鈹€鈹€

  loadHistory: async (quiet = false) => {
    const { currentSessionKey } = get();
    // The Gateway keeps hard-deleted sessions in memory and would re-serve
    // their stale history; suppress loads for sessions deleted this lifetime.
    if (_deletedSessionKeys.has(currentSessionKey)) {
      set({
        messages: [],
        loading: false,
        streamingText: '',
    ...
    const loadPromise = (async () => {
      ...
      const isCurrentSession = () => (
        ...
        && _resetGeneration === loadResetGeneration
      );
      ...
      const applyLoadFailure = (errorMessage: string | null) => {
        ...
        set((state) => {
          ...
          return {
            loading: false,
            // Foreground (manual) refreshes must surface failures even when the
            // message list is non-empty; quiet/background loads stay silent.
      ...
      try {
        ...
        for (let attempt = 0; attempt <= CHAT_HISTORY_STARTUP_RETRY_DELAYS_MS.length; attempt += 1) {
          ...
          const shouldRetry = attempt < CHAT_HISTORY_STARTUP_RETRY_DELAYS_MS.length
            && (isInitialForegroundLoad
              ? shouldRetryStartupHistoryLoad(useGatewayStore.getState().status, errorKind)
              // Manual (non-startup) refreshes get a single retry after 800ms
              // for transient gateway errors instead of silently falling back
              // to stale local data.
              : attempt === 0 && errorKind !== null);
    ...
    try {
      ...
      // Ensure loading state is cleared even when an unhandled error escapes
      // the inner loadPromise (e.g. Gateway RPC timeout, network failure).
      // Without this, the UI spinner would remain stuck indefinitely.
      console.warn('[loadHistory] Unhandled error:', err);
      if (get().currentSessionKey === currentSessionKey) {
  ...
  loadMoreHistory: async () => {
    ...
    try {
      ...
      // Session switched or account changed while loading: drop the result.
      if (get().currentSessionKey !== currentSessionKey || _resetGeneration !== loadResetGeneration) return;
  ...
  sendMessage: async (
    ...
    // A new send legitimately recreates a previously deleted session.
    if (_deletedSessionKeys.delete(targetSessionKey)) {
      persistDeletedSessionKeys(_deletedSessionKeys);
    }
  ...
  // 鈹€鈹€ Handle incoming chat events from Gateway 鈹€鈹€
      ...
      case 'final': {
        ...
        if (finalMsg) {
          ...
          if (isToolResultRole(normalizedFinalMessage.role)) {
            ...
            // tool's stdout (sips / ls / file output) are NOT user-facing —            // the canonical render is the Gateway-injected `assistant-media`
          ...
          set((s) => {
            ...
            if (alreadyExists) {
              // When the message already exists (e.g. from a prior snapshot),
              // merge any pendingToolImages into it instead of discarding them.
              // Without this, files collected from tool.completed events during
              // the run are lost when the final event reuses an existing ID.
              if (pendingImgs.length > 0) {
                const updatedMessages = s.messages.map((m) => {
                  if (m.id !== msgId) return m;
                  const existingKeys = new Set(
                    (m._attachedFiles || []).map(f => f.filePath || f.gatewayUrl).filter(Boolean),
                  );
                  const newFiles = pendingImgs.filter(f => {
                    const key = f.filePath || f.gatewayUrl;
                    return !key || !existingKeys.has(key);
                  });
                  if (newFiles.length === 0) return m;
                  return { ...m, _attachedFiles: [...(m._attachedFiles || []), ...newFiles] };
                });
                return {
          ...
          if (clearLifecycle && !toolOnly) {
            ...
            //
            // We therefore schedule multiple follow-up quiet reloads after
            // every assistant `final` to ensure the `assistant-media` bubble
            // is captured regardless of gateway write latency. The gateway
            // typically persists the bubble 400-1500ms after the streaming
            // final, but under load it can take longer. Progressive retries
            // at 800ms, 2000ms, and 4000ms cover the common cases without
            // excessive RPC overhead (each reload is a cheap in-process call
            // that returns the same snapshot when no new media exists).
            ...
            const MEDIA_CAPTURE_RETRY_DELAYS = [800, 2000, 4000];
            for (const delay of MEDIA_CAPTURE_RETRY_DELAYS) {
              setTimeout(() => {
  ...
  handleRuntimeEvent: (event: ChatRuntimeEvent) => {
    ...
    if (event.type === 'tool.completed' && appliesToActiveUi) {
      ...
      if (files.length > 0) {
        ...
        // Also attach to streaming message for immediate visibility during the run.
        // Without this, files are invisible until loadHistory completes post-final.
        const currentStream = initialState.streamingMessage as RawMessage | null;
        if (currentStream) {
          const existingStreamKeys = new Set(
            (currentStream._attachedFiles || []).map(f => f.filePath || f.gatewayUrl).filter(Boolean),
          );
          const newStreamFiles = files.filter(f => {
            const key = f.filePath || f.gatewayUrl;
            return !key || !existingStreamKeys.has(key);
          });
          if (newStreamFiles.length > 0) {
            nextPatch.streamingMessage = {
              ...currentStream,
              _attachedFiles: [...(currentStream._attachedFiles || []), ...newStreamFiles],
            };
          }
        }
      }
  ...
  // 鈹€鈹€ Refresh: reload history + sessions 鈹€鈹€

  refresh: async () => {
    const { loadHistory, loadSessions, currentSessionKey } = get();
    // Bypass in-flight dedupe so a manual refresh always re-pulls history.
    forceNextHistoryLoad(currentSessionKey);
    set({ error: null, runError: null });
    await Promise.all([loadHistory(), loadSessions(true, true)]);
  },
  ...
  // 鈹€鈹€ Reset for user switch 鈹€鈹€

  resetForUserSwitch: () => {
    // Invalidate every async load started for the previous account — their
    // post-await guards compare against this generation.
    _resetGeneration += 1;
    // Stop in-flight timers so they can never fire into the next account's
    // state, then clear every module-level cache/session tracking map.
    if (_historyPollTimer) {
      clearTimeout(_historyPollTimer);
      _historyPollTimer = null;
    }
    ...
    _loadSessionsInFlight = null;
    _lastLoadSessionsAt = 0;
    _hasResolvedStartupSession = false;
    _historyLoadInFlight.clear();
    _lastHistoryLoadAtBySession.clear();
    _forceNextHistoryLoadBySession.clear();
    _foregroundHistoryLoadSeen.clear();
    _sessionHistoryCache.clear();
    _sessionRunStateCache.clear();
    _thinkingLevelsCache.clear();
    _sendGenerationCounter = 0;
    _activeSendGenerationBySession.clear();
    _chatEventDedupe.clear();
    _lastChatEventAt = 0;
    _imageCache.clear();
    _deletedSessionKeys.clear();
    clearDeletedSessionKeys();

    // Cross-store leftovers that identify the previous account's view.
    clearLastSessionKey();
    clearBaselines();
    clearSessionLabelHydrationTracking();
    ...
    set({
      ...
      currentAgentId: getAgentIdFromSessionKey(DEFAULT_SESSION_KEY),
      sessionLabels: {},
      sessionLastActivity: {},
      thinkingLevel: null,
      thinkingLevels: [],
