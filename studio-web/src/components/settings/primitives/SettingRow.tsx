...
export function SettingRow({
  ...
  return (
    <div className={cn(
      'flex flex-col p-3 sm:p-4 rounded-xl transition-all duration-200',
      'sm:hover:bg-muted/30 sm:hover:translate-x-1',
      className
    )}>
      <div className={cn('flex justify-between gap-4', align === 'start' ? 'items-start' : 'items-center')}>
        <div className={cn('flex gap-3 flex-1', align === 'start' ? 'items-start' : 'items-center')}>
          {Icon && (
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-muted to-muted/50">
