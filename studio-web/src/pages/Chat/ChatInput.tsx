...
import { SendHorizontal, Square, X, Paperclip, FileText, Film, Music, FileArchive, File, FolderOpen, Loader2, AtSign, Search, ChevronDown, Brain } from 'lucide-react';
...
/**
 * i18n label keys for the `/think` level ladder ids.
 */
const THINKING_LEVEL_LABEL_KEYS: Record<string, string> = {
  off: 'composer.thinkingLevelOff',
  low: 'composer.thinkingLevelLow',
  medium: 'composer.thinkingLevelMedium',
  high: 'composer.thinkingLevelHigh',
  xhigh: 'composer.thinkingLevelXHigh',
  adaptive: 'composer.thinkingLevelAdaptive',
  max: 'composer.thinkingLevelMax',
};

/**
 * Fallback level list used only when the gateway session row reports no
 * `thinkingLevels`/`thinkingOptions`. As of 2026 most models support thinking,
 * so the full ladder is offered; the gateway rejects unsupported levels, and
 * once sessions.list returns data the real supported set replaces this list.
 */
const FALLBACK_THINKING_LEVEL_IDS = [
  'off',
  'low',
  'medium',
  'high',
...
function formatThinkingLevelLabel(level: string | null, t: (key: string) => string): string {
  if (!level) return t('composer.thinkingLevelInherit');
  const labelKey = THINKING_LEVEL_LABEL_KEYS[level];
...
export function ChatInput({ onSend, onStop, disabled = false, sending = false }: ChatInputProps) {
  ...
  useEffect(() => {
    if (!pickerOpen && !skillPickerOpen && !modelPickerOpen && !thinkingPickerOpen) return;
    const handlePointerDown = (event: MouseEvent) => {
      ...
      if (!insideAgentPicker && !insideSkillPicker && !insideModelPicker && !insideThinkingPicker) {
        setPickerOpen(false);
  ...
  }, [modelPickerOpen, pickerOpen, skillPickerOpen, thinkingPickerOpen]);
  ...
  // Options are driven by the gateway session row (thinkingLevels). Fall back
  // to the full ladder only when the gateway reported no data yet.
  const thinkingLevelOptions = useMemo(() => {
    if (thinkingLevels.length > 0) {
      return thinkingLevels
        // "minimal" adds nothing over "off" — drop it from the selector.
        .filter((entry) => entry.id !== 'minimal')
        .map((entry) => ({
          value: entry.id,
          // Prefer localized labels for the standard level ladder; keep the
          // gateway-provided label for provider-specific ids.
          label: THINKING_LEVEL_LABEL_KEYS[entry.id]
            ? formatThinkingLevelLabel(entry.id, t)
            : (entry.label?.trim() || entry.id),
        }));
    }
    return FALLBACK_THINKING_LEVEL_IDS.map((id) => ({
      value: id,
      label: formatThinkingLevelLabel(id, t),
    }));
  }, [thinkingLevels, t]);
  ...
  const pickFiles = useCallback(async () => {
    try {
      ...
      if (result.canceled || !result.filePaths?.length) {
        // Fallback to browser-native file picker when the host dialog
        // is unavailable (e.g. studio-web running without Electron).
      ...
      // Fallback to browser-native file picker on error as well.
      fileInputRef.current?.click();
    }
  }, [stagePathFiles]);
  ...
  return (
    <div
      className={cn(
        "w-full flex flex-col p-4 overflow-visible"
      )}
        ...
        <div className={cn(
          ...
          <div className="relative min-h-[48px]">
            {skillTokenRanges.length > 0 && (
              <div
                aria-hidden="true"
                className="pointer-events-none absolute inset-0 z-50 overflow-hidden whitespace-pre-wrap break-words text-sm leading-relaxed text-foreground"
              >
            ...
            <Textarea
              ...
              className={cn(
                'relative w-full min-h-[48px] max-h-[240px] resize-none border-0 focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none bg-transparent p-0 text-sm leading-relaxed placeholder:text-muted-foreground/60',
          ...
          {/* Hidden browser-native file input (fallback when host dialog is unavailable) */}
          <input
            ref={fileInputRef}
            type="file"
            multiple
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1">
            ...
            {showAgentPicker && (
              <div ref={pickerRef} className="relative shrink-0">
                ...
                {pickerOpen && (
                  <div className="absolute left-0 bottom-full z-50 mb-2 w-72 overflow-hidden rounded-2xl border border-black/10 bg-white dark:bg-card p-1.5 shadow-xl dark:border-white/10">
            ...
            <div ref={skillPickerRef} className="relative shrink-0">
              ...
              {skillPickerOpen && (
                <div className="absolute left-0 bottom-full z-50 mb-2 w-80 overflow-hidden rounded-2xl border border-black/10 bg-white dark:bg-card p-1.5 shadow-xl dark:border-white/10">
            ...
            {showModelPicker && (
              <div ref={modelPickerRef} className="relative shrink-0">
                <button
                  ...
                  className={cn(
                    'inline-flex h-8 max-w-[120px] sm:max-w-[220px] items-center gap-1 rounded-lg px-1.5 text-meta font-medium text-muted-foreground transition-colors hover:bg-transparent hover:text-foreground focus-visible:outline-none focus-visible:ring-0 disabled:pointer-events-none disabled:opacity-50',
                ...
                {modelPickerOpen && (
                  <div
                    className="absolute left-0 bottom-full z-50 mb-2 w-72 overflow-hidden rounded-2xl border border-black/10 bg-white dark:bg-card p-1.5 shadow-xl dark:border-white/10"
            ...
            {/* Thinking level selector — session-level, persisted via gateway sessions.patch */}
            <div ref={thinkingPickerRef} className="relative shrink-0">
              <button
                ...
                disabled={inputDisabled || sending}
                title={t('composer.pickThinkingLevel')}
              >
                <Brain className={cn('h-3.5 w-3.5 shrink-0', thinkingLevel && thinkingLevel !== 'off' && 'text-primary')} />
                <span className="hidden sm:inline max-w-[80px] truncate">
                  {formatThinkingLevelLabel(thinkingLevel, t)}
                </span>
                <ChevronDown className={cn('h-3.5 w-3.5 shrink-0 transition-transform', thinkingPickerOpen && 'rotate-180')} />
              </button>
              {thinkingPickerOpen && (
                <div
                  className="absolute left-0 bottom-full z-50 mb-2 w-44 overflow-hidden rounded-2xl border border-black/10 bg-white dark:bg-card p-1.5 shadow-xl dark:border-white/10"
                ...
                >
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {/* First entry clears the session override (inherits agent/global default). */}
                    <button
                      key="inherit"
                      type="button"
                      ...
                      className={cn(
                        'flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium transition-colors',
                        thinkingLevel == null ? 'bg-primary/10 text-foreground' : 'hover:bg-black/5 dark:hover:bg-white/5'
                      )}
                      data-testid="chat-thinking-picker-option-inherit"
                    >
                      <span className="truncate">{t('composer.thinkingLevelInherit')}</span>
                      {thinkingLevel == null && (
                        <span className="h-1.5 w-1.5 rounded-full bg-primary" />
                      )}
                    </button>
                    {thinkingLevelOptions.map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() => void handleSelectThinkingLevel(option.value)}
                        className={cn(
                          'flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium transition-colors',
                          (thinkingLevel ?? null) === option.value ? 'bg-primary/10 text-foreground' : 'hover:bg-black/5 dark:hover:bg-white/5'
                        )}
                        data-testid={`chat-thinking-picker-option-${option.value}`}
                      >
                        <span className="truncate">{option.label}</span>
                        {(thinkingLevel ?? null) === option.value && (
                          <span className="h-1.5 w-1.5 rounded-full bg-primary" />
                        )}
                      </button>
                    ))}
            ...
            >
              {sending ? (
                <Square className="h-5 w-5 sm:h-3.5 sm:w-3.5" fill="currentColor" />
              ) : (
                <SendHorizontal className="h-5 w-5 sm:h-4 sm:w-4" strokeWidth={2} />
              )}
            </Button>
          </div>
        </div>
        {hasFailedAttachments && (
          <div className="mt-2.5 flex items-center justify-end gap-2 text-tiny text-muted-foreground/60 px-4">
            <Button
              variant="link"
              size="sm"
