...
export function Chat() {
  ...
  // Callback for file cards in chat messages — ClawX-style three-tier open:
  //   1. Local file with a local filePath → in-app preview or system open
  //   2. Local directory → shell.openPath
  //   3. Anything else (HTTP URL, gateway-media, etc.) → openAttachment
  //      which resolves to local file or external URL in Main process and
  //      validates the reference every time before opening.
  const handleOpenAttachedFile = useCallback((file: AttachedFileMeta) => {
    // Tier 2: directories always go through shell.openPath so users see them
    // in their file manager rather than the in-app text preview.
    if (file.mimeType === 'application/x-directory' && file.filePath) {
    ...
    // Build a stable AttachmentSourceRef for the unified attachment API.
    // uri is what Main will resolve into a real local path or remote URL.
    // sessionKey/generation are placeholders for the ClawX revalidation
    ...
    const ref = {
      sessionKey: currentSessionKey || 'local',
      generation: 0,
      uri,
    } as const;

    // Tier 1: in-app preview for local files that are user-facing artifacts.
    //   - pdf / xls(x) / docx  → preview
    //   - html / md              → preview
    //   - everything else        → system open (via openAttachment → shell.openPath)
    const previewable = /\.(pdf|xlsx?|docx?|html?|md|markdown)$/i.test(file.fileName)
      && !!file.filePath;
    if (previewable && file.filePath) {
      // Use the in-app preview; this still leaves the system-open path
      // available for users who want the default OS handler.
      const target = buildPreviewTarget(file.filePath, file.fileName, file.fileSize);
      openPreview(target);
      return;
    }

    // Tier 3: delegate to Main. This handles both local files (after
    // canonical-path revalidation) and HTTP URLs (shell.openExternal so the
    // browser downloads rather than renders HTML, per Gateway guidance).
    void hostApi.files.openAttachment(ref)
      .then((result) => {
        if (!result.ok) {
          // Fall back to system open for local files so the user at least
          // gets a chance to interact with the file even on validation
          // failure (e.g. transient permission issues).
          if (file.filePath) {
  ...
  return (
    <div
      ...
      className={cn(
        'relative flex flex-col min-h-0 h-full transition-colors duration-500 text-[13px]',
      )}
    >
      {/* Left column: chat */}
        ...
        <div className="relative flex-1 min-h-0 overflow-visible">
          ...
          {showScrollToLatest && (
            <button
              ...
              className="absolute bottom-4 right-4 z-20 inline-flex items-center gap-2 rounded-full bg-background/95 px-3 py-1.5 text-xs font-medium text-foreground shadow-lg shadow-black/10 backdrop-blur transition-colors hover:bg-black/5 dark:hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 dark:shadow-black/30"
