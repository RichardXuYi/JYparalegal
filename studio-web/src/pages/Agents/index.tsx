...
export function Agents() {
  ...
  return (
    <div data-testid="agents-page" className="flex flex-col h-full w-full dark:bg-background overflow-hidden">
...
const inputClasses = 'h-[44px] rounded-xl font-mono text-meta bg-background border border-black/10 dark:border-white/10 focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary shadow-sm transition-all text-foreground placeholder:text-foreground/40 px-3';
const selectClasses = 'h-[44px] w-full rounded-xl font-mono text-meta bg-background border border-black/10 dark:border-white/10 focus-visible:ring-2 focus-visible:ring-primary/50 focus-visible:border-primary shadow-sm transition-all text-foreground px-3 appearance-none pr-10 bg-[length:16px_16px] bg-[right_12px_center] bg-no-repeat [background-image:url("data:image/svg+xml,%3Csvg%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%20width%3D%2216%22%20height%3D%2216%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%23888%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpath%20d%3D%22m6%209%206%206%206-6%22/%3E%3C/svg%3E")]';
...
function AddAgentDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent className="w-[calc(100%-2rem)] max-w-md rounded-3xl border-0 shadow-2xl bg-surface-modal">
...
function AgentSettingsModal({
  ...
  // Sync the name field during render when the agent changes (React-recommended
  // adjust-during-render pattern) instead of setState-in-effect.
...
function AgentModelModal({
  ...
  // Sync the form selection during render when the model inputs change
  // (React-recommended adjust-during-render pattern) instead of
  // setState-in-effect.
  const [prevModelKey, setPrevModelKey] = useState<{
    overrideModelRef: AgentSummary['overrideModelRef'];
    modelRef: AgentSummary['modelRef'];
    defaultModelRef: typeof defaultModelRef;
    runtimeProviderOptions: RuntimeProviderOption[];
  } | null>(null);
  const modelKeyChanged =
    !prevModelKey
    || prevModelKey.overrideModelRef !== agent.overrideModelRef
    || prevModelKey.modelRef !== agent.modelRef
    || prevModelKey.defaultModelRef !== defaultModelRef
    || prevModelKey.runtimeProviderOptions !== runtimeProviderOptions;
  if (modelKeyChanged) {
    setPrevModelKey({
    ...
    const effective = override ?? splitModelRef(agent.modelRef || defaultModelRef);
    if (effective) {
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && handleRequestClose()}>
      <DialogContent className="z-[60] w-[calc(100%-2rem)] max-w-xl rounded-3xl border-0 shadow-2xl bg-surface-modal">
        <Card>
        <CardHeader className="pb-2">
          <div>
            <DialogTitle asChild>
