...
export const hostApi = {
  ...
  providers: {
    ...
    getModelsMeta: (providerKey: string, modelId: string) => (
      invokeHost('providers', 'getModelsMeta', { providerKey, modelId })
    ),
    updateModelsMeta: (input: { providerKey: string; modelId: string; contextWindow?: number; reasoning?: boolean }) => (
      invokeHost('providers', 'updateModelsMeta', input)
    ),
  },
  files: {
  ...
  media: {
    ...
    resolveGatewayUrl: (input: { gatewayUrl: string }) => invokeHost('media', 'resolveGatewayUrl', input),
  ...
  skills: {
    ...
    sync: () => invokeHost('skills', 'sync'),
    syncStatus: () => invokeHost('skills', 'syncStatus'),
    upload: (input: SkillUploadPayload) => invokeHost('skills', 'upload', input),
  ...
  sync: {
    status: (payload?: SyncStatusPayload) => invokeHost('sync', 'status', payload ?? {}),
    upload: (payload: SyncRunPayload) => invokeHost('sync', 'upload', payload),
    download: (payload: SyncRunPayload) => invokeHost('sync', 'download', payload),
  },
  auth: {
    ...
    listDevices: () => invokeHost('auth', 'listDevices'),
    revokeDevice: (deviceId: number) => invokeHost('auth', 'revokeDevice', { deviceId }),
  },
};
