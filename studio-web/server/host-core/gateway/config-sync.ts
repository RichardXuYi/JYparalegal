...
function readIntEnvLocal(name: string, fallback: number): number {
...
import { isLegacyScope, getPersistedAccessToken } from '../utils/user-scope';
...
// 鈹€鈹€ Auto-upgrade bundled plugins on startup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
const CHANNEL_PLUGIN_MAP: Record<string, { dirName: string; npmName: string }> = {
  ...
  [CLAWX_OPENAI_IMAGE_PROVIDER_KEY]: { dirName: CLAWX_OPENAI_IMAGE_PROVIDER_KEY, npmName: 'grandpoem-studio-openai-image-plugin' },
};

/**
 * OpenClaw ships some channel plugins as bundled extensions under
 * dist/extensions/. If GrandPoem Paralegal Studio previously mirrored one of those ids into
  ...
  for (const channelType of configuredChannels) {
    ...
    if (bundledDir) {
      ...
      if (!isInstalled || (sourceVersion && installedVersion && sourceVersion !== installedVersion)) {
        logger.info(`[plugin] ${isInstalled ? 'Auto-upgrading' : 'Installing'} ${channelType} plugin${isInstalled ? `: ${installedVersion} 鈫?${sourceVersion}` : `: ${sourceVersion}`} (bundled)`);
        try {
    ...
    if (!app.isPackaged) {
      ...
      if (npmPkgPath && existsSync(fsPath(join(npmPkgPath, 'openclaw.plugin.json')))) {
        ...
        logger.info(`[plugin] ${isInstalled ? 'Auto-upgrading' : 'Installing'} ${channelType} plugin${isInstalled ? `: ${installedVersion} 鈫?${sourceVersion}` : `: ${sourceVersion}`} (dev/node_modules)`);
 ...
 *   dist/node_modules/ 鈫?openclaw/node_modules/ 鈫?— * It does NOT search `dist/extensions/telegram/node_modules/`.
 *
...
// 鈹€鈹€ Pre-launch sync 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
export async function syncGatewayConfigBeforeLaunch(
  ...
  try {
    ...
    // Always refresh trusted install metadata through GrandPoem Paralegal Studio —this must not
...
/**
 * Seed a freshly created per-account state dir with its own default
 * workspace so agents never fall back to another account's files. The
...
function ensureScopedStateDirInitialized(businessTools: boolean): void {
  ...
  if (existsSync(fsPath(configFile))) return;
  try {
...
function buildJyparalegalMcpServer(): Record<string, unknown> {
  const base = process.env.BACKEND_URL?.trim() || 'http://localhost:8181';
  const token = getPersistedAccessToken();
  const server: Record<string, unknown> = {
    url: `${base}/internal/tools`,
    transport: 'streamable-http',
    description: 'JYparalegal 法律业务工具（签署/审查/抽取/起草/KB）；create_sign_task 需 HITL 二次确认',
  };
  if (token) {
...
function ensureJyparalegalMcpServer(businessTools: boolean): void {
  ...
  const stateDir = getOpenClawConfigDir();
  const configFile = join(stateDir, 'openclaw.json');
  if (!existsSync(fsPath(configFile))) return; // 首次创建由 seed 负责
  try {
    ...
    cfg.mcp = mcp;
    writeFileSync(fsPath(configFile), `${JSON.stringify(cfg, null, 2)}\n`, 'utf-8');
    logger.info(`[mcp] Ensured mcp.servers.jyparalegal (token=${next.headers ? 'yes' : 'no'})`);
  } catch (err) {
    logger.warn('[mcp] Failed to ensure jyparalegal MCP server entry:', err);
  }
}
...
export async function prepareGatewayLaunchContext(port: number): Promise<GatewayLaunchContext> {
  ...
  const forkEnv: Record<string, string | undefined> = {
    ...
    // Pin the Gateway to the active account's state dir so per-user data
    // (config, agents, sessions, workspaces) stays isolated (see user-scope.ts).
    ...
    // Heap cap for the Gateway process. The fleet supervisor passes
    // JY_GATEWAY_MAX_OLD_SPACE_MB down from FLEET_GATEWAY_MAX_OLD_SPACE_MB;
    // the desktop leaves the env unset and uses the default below. A runaway
    // Gateway OOMs only itself — the manager's crash machinery restarts it.
    NODE_OPTIONS: `--max-old-space-size=${readIntEnvLocal('JY_GATEWAY_MAX_OLD_SPACE_MB', 1024)}`,
  };
