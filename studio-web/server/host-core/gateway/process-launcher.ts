...
(function () {
  ...
  if (globalThis.__grandpoemStudioFetchPatched) return;
  globalThis.__grandpoemStudioFetchPatched = true;

  globalThis.fetch = function grandpoemStudioFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      flat['X-OpenRouter-Title'] = 'GrandPoem Paralegal Studio';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__grandpoemStudioPatched) {
        cp.__grandpoemStudioPatched = true;
...
export async function launchGatewayProcess(options: {
  ...
  // which causes:
  //   - cross-machine name collisions when multiple OpenClaw/GrandPoem Paralegal Studio peers
  //     share a LAN (each falls back to "<name> (OpenClaw) (2)")
  //   - self-collisions on multi-homed hosts (Wi-Fi + Tailscale + utun ...)
  //   - "ghost" record collisions after an unclean GrandPoem Paralegal Studio exit, because
  //     SIGKILL prevents ciao from emitting the mDNS goodbye record.
  //
  // GrandPoem Paralegal Studio has no UI for LAN gateway discovery today, so the advertiser is
