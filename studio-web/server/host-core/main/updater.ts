...
// Security: use HTTPS to protect update channel from MITM tampering.
// If the OSS endpoint lacks a valid TLS certificate, deploy one before
// shipping; do not revert to plaintext HTTP.
const OSS_BASE_URL = 'https://47.116.163.57:8082';
 ...
 * e.g. "0.1.8-alpha.0" 鈫?"alpha", "1.0.0-beta.1" 鈫?"beta", "1.0.0" 鈫?"latest"
 */
...
export class AppUpdater extends EventEmitter {
  ...
  constructor() {
    ...
    // e.g. channel "alpha" 鈫?requests alpha-mac.yml, channel "latest" 鈫?requests latest-mac.yml
   ...
   *
   * GrandPoem Paralegal Studio uses a prompt-first update flow: finding an update shows a UI prompt,
