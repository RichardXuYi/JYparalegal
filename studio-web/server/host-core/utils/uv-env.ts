...
function buildGrandPoemStudioUvConfigToml(): string {
  return [
    '# This file is managed by GrandPoem Paralegal Studio.',
    '# It lets uv use GrandPoem Paralegal Studio-selected mirrors even when host exec filters UV_INDEX_URL.',
...
export function getGrandPoemStudioUvConfigFilePath(): string {
  return path.join(getOpenClawConfigDir(), 'grandpoem-studio', 'uv.toml');
}

function ensureGrandPoemStudioUvConfigFile(): string | null {
  const filePath = getGrandPoemStudioUvConfigFilePath();
  const content = buildGrandPoemStudioUvConfigToml();
  ...
  try {
    ...
    logger.warn('Failed to write GrandPoem Paralegal Studio uv config file:', err);
    return null;
  }
}
...
export async function getUvMirrorEnv(): Promise<Record<string, string>> {
  ...
  const uvConfigFile = ensureGrandPoemStudioUvConfigFile();
  return uvConfigFile
