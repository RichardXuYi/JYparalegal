 ...
 * the Electron IPC system to display UI in the GrandPoem Paralegal Studio frontend.
 */
...
class DeviceOAuthManager extends EventEmitter {
    ...
    private async onSuccess(providerType: OAuthProviderType, token: {
        ...
        //    (matches what `openclaw models auth login` 鈫?upsertAuthProfile writes).
        ...
        // 3. Save provider record in GrandPoem Paralegal Studio's own store so UI shows it as configured
