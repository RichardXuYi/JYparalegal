...
async function parseSkillFrontmatterName(manifestPath: string): Promise<string | null> {
  try {
    const fileStat = await stat(manifestPath);
    if (fileStat.size > MAX_SKILL_FILE_BYTES) return null;
    const content = await readFile(manifestPath, 'utf-8');
    if (!content.startsWith('---')) return null;
    const endIndex = content.indexOf('\n---', 3);
    if (endIndex === -1) return null;
...
async function inspectSkillDir(params: {
  ...
  try {
    ...
    const frontmatterName = await parseSkillFrontmatterName(manifestPath);
    return {
      name: frontmatterName || basename(skillDirRealPath),
      description,
...
export function filterEnabledQuickAccessSkills(
  ...
  return skills.filter((skill) => {
    const skillNameKey = normalizeLookupKey(skill.name);
    const dirNameKey = normalizeLookupKey(basename(skill.baseDir));
    const normalizedPath = resolve(skill.baseDir);
    const lookupKeys = [skillNameKey, dirNameKey].filter(Boolean);

    if (
      lookupKeys.some((key) => disabledKeys.has(key) || configDisabledKeys.has(key))
    ...
    if (!runtimeSkills || runtimeSkills.length === 0) {
      if (configEnabledKeys.size > 0) {
        return lookupKeys.some((key) => configEnabledKeys.has(key));
      }
      return true;
    }

    if (
      lookupKeys.some((key) => enabledKeys.has(key) || configEnabledKeys.has(key))
