...
const CLAWX_BEGIN = '<!-- grandpoem-studio:begin -->';
const CLAWX_END = '<!-- grandpoem-studio:end -->';
...
export function buildDefaultGrandPoemStudioIdentityContent(): string {
  return [
    '# IDENTITY.md - GrandPoem Paralegal Studio',
    '',
    '- **Name:** GrandPoem Paralegal Studio',
    '- **Creature:** desktop AI assistant',
    '- **Vibe:** concise, capable, and practical',
    '- **Emoji:** 馃惥',
    '- **Avatar:**',
    '',
    'GrandPoem Paralegal Studio uses a default desktop identity instead of chat-first bootstrap.',
    '',
  ].join('\n');
}
...
/**
 * Ensure GrandPoem Paralegal Studio-managed workspaces have a non-template IDENTITY.md before the
 * Gateway initializes them. Existing custom identities are preserved.
 */
export async function ensureGrandPoemStudioIdentityFile(
  workspaceDir: string,
  ...
  const defaultIdentity = buildDefaultGrandPoemStudioIdentityContent();
  ...
  if (await fileExists(bootstrapPath)) {
    try {
      ...
      logger.info(`Removed chat-first bootstrap file from GrandPoem Paralegal Studio workspace (${resolvedWorkspaceDir})`);
    } catch {
      logger.warn(`Failed to remove chat-first bootstrap file: ${bootstrapPath}`);
    }
  } else if (wroteIdentity) {
    logger.info(`Seeded default GrandPoem Paralegal Studio identity for workspace (${resolvedWorkspaceDir})`);
  }
}

export async function ensureGrandPoemStudioDefaultIdentity(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    await ensureGrandPoemStudioIdentityFile(workspaceDir, { createDir: waitForGatewaySeed });
  }
}

// 鈹€鈹€ Pure helpers (no I/O) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

/**
 * Merge a GrandPoem Paralegal Studio context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeGrandPoemStudioSection(existing: string, section: string): string {
 ...
 * for GrandPoem Paralegal Studio-managed workspaces.  Removes everything from the heading
...
// 鈹€鈹€ Workspace directory resolution 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€ Bootstrap file repair 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

/**
 * Detect and remove bootstrap .md files that contain only GrandPoem Paralegal Studio markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairGrandPoemStudioOnlyBootstrapFiles(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
    ...
    for (const file of entries) {
      ...
      if (before === '' && after === '') {
        try {
          ...
          logger.info(`Removed GrandPoem Paralegal Studio-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove GrandPoem Paralegal Studio-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
// 鈹€鈹€ Context merging 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

/**
 * Merge GrandPoem Paralegal Studio context snippets into workspace bootstrap files that already
...
type EnsureGrandPoemStudioContextOptions = {
  /**
...
async function mergeGrandPoemStudioContextOnce(options: EnsureGrandPoemStudioContextOptions = {}): Promise<MergeResult> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('GrandPoem Paralegal Studio context directory not found, skipping context merge');
    return { missing: 0, retryableMissing: 0 };
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.grandpoem-studio.md'));
  } catch {
    return { missing: 0, retryableMissing: 0 };
  }
  ...
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.grandpoem-studio.md', '.md');
      ...
      const merged = mergeGrandPoemStudioSection(existing, section);
      // Compare against on-disk content so we persist changes even when only
      // First Run stripping happened and the GrandPoem Paralegal Studio section stayed identical.
      if (merged !== originalExisting) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged GrandPoem Paralegal Studio context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
const MAX_RETRIES = 5;
let ensureGrandPoemStudioContextPromise: Promise<void> | null = null;
let ensureGrandPoemStudioContextWaitsForAll = false;

/**
 * Ensure GrandPoem Paralegal Studio context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureGrandPoemStudioContext(options: EnsureGrandPoemStudioContextOptions = {}): Promise<void> {
  if (ensureGrandPoemStudioContextPromise) {
    if (options.waitForAllConfiguredWorkspaces && !ensureGrandPoemStudioContextWaitsForAll) {
      return ensureGrandPoemStudioContextPromise.then(() => ensureGrandPoemStudioContext(options));
    }
    return ensureGrandPoemStudioContextPromise;
  }

  ensureGrandPoemStudioContextWaitsForAll = options.waitForAllConfiguredWorkspaces === true;
  ensureGrandPoemStudioContextPromise = runEnsureGrandPoemStudioContext(options).finally(() => {
    ensureGrandPoemStudioContextPromise = null;
    ensureGrandPoemStudioContextWaitsForAll = false;
  });
  return ensureGrandPoemStudioContextPromise;
}

async function runEnsureGrandPoemStudioContext(options: EnsureGrandPoemStudioContextOptions): Promise<void> {
  let result = await mergeGrandPoemStudioContextOnce(options);
  if (result.retryableMissing === 0) {
    if (result.missing > 0) {
      logger.debug(`GrandPoem Paralegal Studio context merge skipped ${result.missing} non-ready file(s)`);
    }
    return;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    result = await mergeGrandPoemStudioContextOnce(options);
    if (result.retryableMissing === 0) {
      logger.info(`GrandPoem Paralegal Studio context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`GrandPoem Paralegal Studio context merge: ${result.retryableMissing} startup file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`GrandPoem Paralegal Studio context merge: ${result.retryableMissing} startup file(s) still missing after ${MAX_RETRIES} retries`);
}
