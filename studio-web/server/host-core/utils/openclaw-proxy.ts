...
interface SyncProxyOptions {
   ...
   * currently disabled in GrandPoem Paralegal Studio settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync GrandPoem Paralegal Studio global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because GrandPoem Paralegal Studio proxy is disabled and preserve mode is enabled');
      return;
    }
