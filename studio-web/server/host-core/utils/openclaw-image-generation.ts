...
import { ensureGrandPoemStudioOpenAiImagePluginInstalled } from './plugin-install';
...
/** Some relays (e.g. gpt-image-2) reject 512脳512 as below minimum pixel budget. */
...
export async function setImageGenerationConfig(
  ...
  return withConfigLock(async () => {
    ...
    // GrandPoem Paralegal Studio image generation is configured as one explicit custom endpoint.
    // Keep OpenClaw from appending other authenticated image providers such as
    // minimax-portal/image-01 after the configured GrandPoem Paralegal Studio image provider.
...
export async function applyOpenAiImageRelaySettings(params: {
  ...
  if (params.enabled) {
    ensureGrandPoemStudioOpenAiImagePluginInstalled();
  }
}
