...
import { ensureGrandPoemStudioIdentityFile } from './openclaw-workspace';
...
function getDefaultWorkspacePath(config: AgentConfigDocument): string {
  ...
  return typeof defaults?.workspace === 'string' && defaults.workspace.trim()
    ? defaults.workspace
    // Resolved lazily against the active user scope (see user-scope.ts).
    : join(getOpenClawConfigDir(), 'workspace');
}
...
async function provisionAgentFilesystem(
  ...
  // on first use, but GrandPoem Paralegal Studio still pre-seeds IDENTITY.md so desktop workspaces
  ...
  await ensureGrandPoemStudioIdentityFile(targetWorkspace, { createDir: true });
