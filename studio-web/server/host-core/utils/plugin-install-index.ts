/**
 * Persist GrandPoem Paralegal Studio-managed plugin install records into OpenClaw's SQLite
...
function resolveOpenClawStateDir(): string {
  return process.env.OPENCLAW_STATE_DIR?.trim() || getOpenClawConfigDir();
}
 ...
 * GrandPoem Paralegal Studio-authored records win over stale SQLite entries for the same plugin id.
 */
    ...
    if (row) {
      ...
      db.prepare(`
        INSERT INTO installed_plugin_index (
          ...
          ?, 1, 'grandpoem-studio-managed', 'grandpoem-studio-managed',
          1, 'grandpoem-studio-managed', ?, 'source-changed',
          ?, '[]', '[]', ?, ?
        )
      `).run(
