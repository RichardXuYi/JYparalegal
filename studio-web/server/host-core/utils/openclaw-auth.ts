...
import { inferCustomModelContextWindow, inferCustomModelInputModalities, inferCustomModelReasoning, resolveContextWindowPresets } from '../shared/providers/model-capabilities';
import {
...
// 鈹€鈹€ Auth Profiles I/O 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
 ...
 *
 * This intentionally reads auth-profiles.json rather than GrandPoem Paralegal Studio's provider
...
// 鈹€鈹€ Agent Discovery 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€ OpenClaw Config Helpers 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
/**
 * Backfill `reserveTokensFloor` on compaction configs that GrandPoem Paralegal Studio or OpenClaw
...
/**
 * Self-heal helper: walk `models.providers.*` entries for providers whose
 * model metadata is managed locally (custom / ollama) and fill in an inferred
 * `contextWindow` on model rows that have neither `contextWindow` nor
 * `contextTokens`. Rows written by older GrandPoem Paralegal Studio versions only carried
 * `{ id, name, input }`, which disables OpenClaw's preemptive compaction and
 * context-window guard for those providers.
 *
 * Deliberately scoped to `custom-` and `ollama` keys: registry providers own
 * their metadata, and small local models keep their explicit values when set.
 */
function backfillCustomProviderModelContextWindows(config: Record<string, unknown>): string[] {
  const models = (config.models || {}) as Record<string, unknown>;
  const providers = (models.providers || {}) as Record<string, unknown>;
  const backfilled: string[] = [];
  const isLocallyManagedProviderKey = (key: string): boolean =>
    key.startsWith('custom-') || key.startsWith('ollama');
  ...
  for (const [providerKey, entry] of Object.entries(providers)) {
    ...
    for (const row of rows) {
      ...
      let changed = false;
      if (
        isLocallyManagedProviderKey(providerKey)
      ...
      // Most 2026 models support thinking; rows without an explicit
      // `reasoning` default to false in OpenClaw, silently disabling the
      // thinking-mode feature. Backfill the inferred value on every row.
      if (typeof row.reasoning !== 'boolean') {
        row.reasoning = inferCustomModelReasoning(row.id);
        changed = true;
      }
      if (changed) {
...
// 鈹€鈹€ Exported Functions (all async) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
export async function removeProviderFromOpenClaw(provider: string): Promise<void> {
  ...
  // provider key via AUTH_PROFILE_PROVIDER_KEY_MAP (e.g. "openai-codex" 鈫?"openai").
 ...
 * GrandPoem Paralegal Studio-written entries historically only included `{ id, name }`.
 *
 ...
 * The bundled OpenClaw distribution GrandPoem Paralegal Studio ships does not register that harness,
  ...
  const runtimeModels = (options.modelIds ?? []).map((id) => ({
    ...
    ...(options.inferRuntimeModelInputs
      ? {
        ...
        // reasoning:true registers the model as thinking-capable; rows without
        // it default to reasoning:false and lose the thinking-mode feature.
        reasoning: inferCustomModelReasoning(id),
      }
      : {}),
  }));
 ...
 * (written by earlier GrandPoem Paralegal Studio builds that did not pin the runtime) get
 * repaired before the next Gateway reload picks them up. Without this, users
 * who upgrade GrandPoem Paralegal Studio while still pointing at an OpenAI provider would keep
...
/**
 * Configure a GrandPoem Paralegal Studio-owned OpenAI-compatible image provider.
...
export function readOpenAiCompatibleImageRelayState(
  ...
  const grandpoemStudioRelay = readModelsProvider(config, CLAWX_OPENAI_IMAGE_PROVIDER_KEY);
  const relayBaseUrl = typeof grandpoemStudioRelay?.baseUrl === 'string' ? grandpoemStudioRelay.baseUrl.trim() : '';
  if (relayBaseUrl) {
    return { enabled: true, baseUrl: relayBaseUrl, providerKey: CLAWX_OPENAI_IMAGE_PROVIDER_KEY };
  }

  // Backward compatibility for GrandPoem Paralegal Studio builds that used models.providers.openai
  // for image relay. New saves move to the GrandPoem Paralegal Studio-owned provider above.
...
export async function getActiveOpenClawProviders(): Promise<Set<string>> {
  ...
  try {
    ...
    //    the provider prefix (e.g. "modelstudio/qwen3.6-plus" 鈫?"modelstudio").
 ...
 * Used by GrandPoem Paralegal Studio to seed the provider store when it's empty but providers are
...
/**
 * Write the GrandPoem Paralegal Studio gateway token into ~/.openclaw/openclaw.json.
 */
 ...
 * which means conversations disappear after roughly one day.  GrandPoem Paralegal Studio sets
...
export async function batchSyncConfigFields(token: string): Promise<void> {
  ...
  return withConfigLock(async () => {
    ...
    // 鈹€鈹€ Gateway token + controlUi 鈹€鈹€
    const gateway = (
      config.gateway && typeof config.gateway === 'object'
        ? { ...(config.gateway as Record<string, unknown>) }
        : {}
    ...
    // 鈹€鈹€ web_fetch SSRF policy (fake-IP / transparent-proxy environments) 鈹€鈹€
    ...
    // 鈹€鈹€ Compaction safeguard default 鈹€鈹€
    ...
    // 鈹€鈹€ Custom provider contextWindow backfill 鈹€鈹€
...
async function updateModelsJsonProviderEntriesForAgents(
  ...
  for (const agentId of agentIds) {
    ...
    const mergedModels = (entry.models ?? []).map((m) => {
      ...
      // Custom/ollama-provider rows need an explicit contextWindow so the
      // embedded runner can budget compaction (see backfillCustomProviderModelContextWindows).
      if (
        (providerType.startsWith('custom-') || providerType.startsWith('ollama'))
...
export interface ModelEntryMetadata {
  contextWindow?: number;
  contextTokens?: number;
  reasoning?: boolean;
}

/** `readModelEntryMetadata` result enriched with UI-facing inference data. */
export interface ModelEntryMetadataWithPresets extends ModelEntryMetadata {
  /** Inferred context window for the model id (used when no explicit value is stored). */
  inferredContextWindow: number;
  /** Inferred thinking support for the model id (used when no explicit value is stored). */
  inferredReasoning: boolean;
  /** Context-window tiers the model is allowed to pick from. */
  presets: number[];
}
...
function findModelRow(
  ...
  const rows = Array.isArray(entry.models) ? (entry.models as Array<Record<string, unknown>>) : [];
  return rows.find((candidate) => isPlainRecord(candidate) && candidate.id === modelId);
}

/**
 * Read a single model row's metadata (`contextWindow` / `contextTokens` /
 * `reasoning`) from openclaw.json `models.providers.<providerKey>.models[]`,
 * enriched with the inferred context window and selectable presets for the
 * model-settings UI. Returns null when the provider or model row is not
 * present.
 */
export async function readModelEntryMetadata(
  providerKey: string,
  modelId: string,
): Promise<ModelEntryMetadataWithPresets | null> {
  return withConfigLock(async () => {
    const config = await readOpenClawJson();
    const providers = (config.models as Record<string, unknown> | undefined)?.providers;
    if (!providers || typeof providers !== 'object') return null;
    const entry = (providers as Record<string, unknown>)[providerKey];
    const row = findModelRow(
      isPlainRecord(entry) ? (entry as Record<string, unknown>) : undefined,
      modelId,
    );
    if (!row) return null;
    const metadata: ModelEntryMetadataWithPresets = {
      inferredContextWindow: inferCustomModelContextWindow(modelId),
      inferredReasoning: inferCustomModelReasoning(modelId),
      presets: resolveContextWindowPresets(modelId),
    };
    if (typeof row.contextWindow === 'number') {
      metadata.contextWindow = row.contextWindow;
      // Keep an explicit stored value selectable even when it is not part of
      // the inferred tier list (e.g. a catalog value of 1M for deepseek-v4).
      if (!metadata.presets.includes(row.contextWindow)) {
        metadata.presets = [...metadata.presets, row.contextWindow].sort((a, b) => a - b);
      }
    }
    if (typeof row.contextTokens === 'number') metadata.contextTokens = row.contextTokens;
    if (typeof row.reasoning === 'boolean') metadata.reasoning = row.reasoning;
    return metadata;
  });
}

/**
 * Patch a model row's metadata (`contextWindow` / `reasoning`) in openclaw.json
 * and in every discovered agent's `models.json`. Missing model rows are
 * created so the UI setting always lands on disk; all other row fields are
 * preserved. Returns whether at least one config file was actually modified.
 */
export async function updateModelEntryMetadata(
  providerKey: string,
  modelId: string,
  metadata: ModelEntryMetadata,
): Promise<{ updated: boolean }> {
  const hasPatch = metadata.contextWindow !== undefined || metadata.reasoning !== undefined;
  if (!hasPatch) {
    return { updated: false };
  }

  const patchRow = (row: Record<string, unknown>): boolean => {
    let changed = false;
    if (metadata.contextWindow !== undefined && row.contextWindow !== metadata.contextWindow) {
      row.contextWindow = metadata.contextWindow;
      changed = true;
    }
    if (metadata.reasoning !== undefined && row.reasoning !== metadata.reasoning) {
      row.reasoning = metadata.reasoning;
      changed = true;
    }
    return changed;
  };

  const upsertRow = (rows: Array<Record<string, unknown>>): { rows: Array<Record<string, unknown>>; changed: boolean } => {
    const row = rows.find((candidate) => candidate.id === modelId);
    if (row) {
      return { rows, changed: patchRow(row) };
    }
    const created: Record<string, unknown> = { id: modelId, name: modelId };
    patchRow(created);
    return { rows: [...rows, created], changed: true };
  };

  return withConfigLock(async () => {
    let updated = false;

    // Patch openclaw.json's provider entry.
    const config = await readOpenClawJson();
    ...
    const providers = (models.providers && typeof models.providers === 'object'
      ? models.providers as Record<string, Record<string, unknown>>
      : {});
    const entry = providers[providerKey] && typeof providers[providerKey] === 'object'
      ? { ...providers[providerKey] }
      : { baseUrl: undefined, api: undefined };
    const rows = Array.isArray(entry.models)
      ? (entry.models as Array<Record<string, unknown>>).map((candidate) => ({ ...candidate }))
      : [];
    const result = upsertRow(rows);
    if (result.changed) {
      entry.models = result.rows;
      providers[providerKey] = entry;
      models.providers = providers;
      config.models = models;
      await writeOpenClawJson(config);
    ...
    // Patch every discovered agent's models.json.
    const agentIds = await discoverAgentIds();
    for (const agentId of agentIds) {
      const modelsPath = join(getOpenClawConfigDir(), 'agents', agentId, 'agent', 'models.json');
      let data: Record<string, unknown> = {};
      try {
        ...
        // corrupt / missing —nothing to patch for this agent
      }
      const agentProviders = (data.providers && typeof data.providers === 'object'
        ? data.providers as Record<string, Record<string, unknown>>
        : {});
      const agentEntry = agentProviders[providerKey] && typeof agentProviders[providerKey] === 'object'
        ? { ...agentProviders[providerKey] }
        : {};
      const agentRows = Array.isArray(agentEntry.models)
        ? (agentEntry.models as Array<Record<string, unknown>>).map((candidate) => ({ ...candidate }))
        : [];
      const agentResult = upsertRow(agentRows);
      if (!agentResult.changed) continue;
      agentEntry.models = agentResult.rows;
      agentProviders[providerKey] = agentEntry;
      data.providers = agentProviders;
      try {
        await writeJsonFile(modelsPath, data);
        updated = true;
        console.log(`Patched model metadata for agent "${agentId}" provider "${providerKey}" model "${modelId}"`);
      } catch (err) {
        console.warn(`Failed to patch model metadata for agent "${agentId}":`, err);
      }
    }
 ...
 * OpenClaw/GrandPoem Paralegal Studio versions or external tools.
 *
...
// GrandPoem Paralegal Studio desktop does not expose the subagent workflow. Deny the spawn entry
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // which coalesces null 鈫?{}.  We need to distinguish a genuinely empty
    ...
    // 鈹€鈹€ skills section 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    // 鈹€鈹€ plugins section 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    // 鈹€鈹€ tools.web.search.kimi 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    // 鈹€鈹€ tools.profile & sessions.visibility 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    // OpenClaw 3.8+ requires tools.profile = 'full' and tools.sessions.visibility = 'all'
    // for GrandPoem Paralegal Studio to properly integrate with its updated tool system.
    ...
    // GrandPoem Paralegal Studio keeps direct skill-creator authoring instead, so deny the workshop
    ...
    if (denyResult.modified) {
      ...
      toolsModified = true;
      console.log('[sanitize] Added "skill_workshop" to tools.deny for GrandPoem Paralegal Studio desktop');
    } else if (!Array.isArray(toolsConfig.deny) || toolsConfig.deny.length !== denyResult.deny.length) {
      toolsConfig.deny = denyResult.deny;
      toolsModified = true;
    }

    // GrandPoem Paralegal Studio desktop does not expose the subagent workflow. Deny the spawn
    ...
    if (subagentDenyResult.modified) {
      ...
      toolsModified = true;
      console.log(`[sanitize] Added subagent tool(s) to tools.deny for GrandPoem Paralegal Studio desktop: ${SUBAGENT_TOOL_DENY_ENTRIES.join(', ')}`);
    } else if (!Array.isArray(toolsConfig.deny) || toolsConfig.deny.length !== subagentDenyResult.deny.length) {
      toolsConfig.deny = subagentDenyResult.deny;
      toolsModified = true;
    }

    // 鈹€鈹€ tools.exec approvals (OpenClaw 3.28+) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    // GrandPoem Paralegal Studio is a local desktop app where the user is the trusted operator.
    ...
    if (execConfig.security !== 'full' || execConfig.ask !== 'off') {
      ...
      toolsModified = true;
      console.log('[sanitize] Set tools.exec.security="full" and tools.exec.ask="off" to disable exec approvals for GrandPoem Paralegal Studio desktop');
    }
    ...
    // 鈹€鈹€ session.dmScope 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    // OpenClaw defaults DM session routing to "main" (all channels share
    // agent:main:main), which makes GrandPoem Paralegal Studio sidebar conflate feishu, dingtalk,
    ...
    if (sessionConfig.dmScope !== 'per-channel-peer' && sessionConfig.dmScope !== 'per-account-channel-peer') {
      ...
      modified = true;
      console.log('[sanitize] Set session.dmScope="per-channel-peer" so channel DMs appear as separate sessions in GrandPoem Paralegal Studio');
    }

    // 鈹€鈹€ Skill Workshop hard-disable (OpenClaw 6.10+) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    const gateway = (
      config.gateway && typeof config.gateway === 'object'
        ? { ...(config.gateway as Record<string, unknown>) }
        : {}
    ...
    if (gatewayDenyResult.modified) {
      ...
      console.log('[sanitize] Added "skill_workshop" to gateway.tools.deny for GrandPoem Paralegal Studio desktop');
    ...
    if (gatewaySubagentDenyResult.modified) {
      ...
      gatewayModified = true;
      console.log(`[sanitize] Added subagent tool(s) to gateway.tools.deny for GrandPoem Paralegal Studio desktop: ${SUBAGENT_TOOL_DENY_ENTRIES.join(', ')}`);
    ...
    if (autonomous.enabled !== false) {
      ...
      skillsModified = true;
      console.log('[sanitize] Disabled skills.workshop.autonomous for GrandPoem Paralegal Studio desktop');
    }
    ...
    if (skillCreatorEntry.enabled !== true) {
      ...
      skillsModified = true;
      console.log('[sanitize] Enabled bundled skill-creator for direct skill authoring in GrandPoem Paralegal Studio desktop');
    }
    ...
    // 鈹€鈹€ plugins.entries.feishu cleanup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    if (typeof plugins === 'object' && !Array.isArray(plugins)) {
      ...
      // 鈹€鈹€ MiniMax merged-plugin compatibility cleanup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      ...
      // 鈹€鈹€ acpx legacy config/install cleanup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      ...
      // 鈹€鈹€ wecom-openclaw-plugin 鈫?wecom migration 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      ...
      if (Array.isArray(pluginsObj.allow)) {
        ...
        if (legacyIdx !== -1) {
          ...
          console.log(`[sanitize] Migrated plugins.allow: ${LEGACY_WECOM_ID} 鈫?${NEW_WECOM_ID}`);
          modified = true;
        }
      }
      if (pEntries?.[LEGACY_WECOM_ID]) {
        ...
        console.log(`[sanitize] Migrated plugins.entries: ${LEGACY_WECOM_ID} 鈫?${NEW_WECOM_ID}`);
        modified = true;
      }

      // 鈹€鈹€ qqbot built-in channel cleanup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      ...
      // 鈹€鈹€ qwen-portal 鈫?modelstudio migration 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      ...
      // 鈹€鈹€ Remove legacy built-in 'feishu' registration 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
      // GrandPoem Paralegal Studio bundles Feishu via the official @larksuite/openclaw-lark
      ...
      // 鈹€鈹€ Reconcile built-in channels with restrictive plugin allowlists 鈹€鈹€
      ...
      // GrandPoem Paralegal Studio-critical bundled plugins, active provider plugins, and explicitly
    ...
    // 鈹€鈹€ channels default-account migration and cleanup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
