...
export function createAgentsApi(ctx: AgentsApiContext): CompleteHostServiceRegistry['agents'] {
  return {
    ...
    create: async (payload) => {
      ...
      void ensureGrandPoemStudioContext({ waitForAllConfiguredWorkspaces: true }).catch((err) => {
        console.warn('[agents] Failed to ensure GrandPoem Paralegal Studio context after agent creation:', err);
      });
