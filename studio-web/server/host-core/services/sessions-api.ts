...
async function deleteSession(sessionKey: string, gatewayManager?: GatewayManager): Promise<{ success: boolean; error?: string }> {
  ...
  // After file deletion, attempt to refresh the Gateway's session cache by
  // calling sessions.list. The OpenClaw Gateway does not expose a dedicated
  // sessions.delete RPC, but sessions.list should re-read from disk and
  // surface the updated list to the frontend.
  if (gatewayManager) {
    try {
      ...
      logger.info(`[session:delete] Triggered gateway sessions.list refresh for ${sessionKey}`);
    } catch (error) {
      logger.warn(`[session:delete] Failed to refresh gateway session cache for ${sessionKey}: ${String(error)}`);
    }
  }
...
export function createSessionsApi(options?: { gatewayManager?: GatewayManager }): CompleteHostServiceRegistry['sessions'] {
  const gatewayManager = options?.gatewayManager;
  return {
    delete: async (payload) => deleteSession(getSessionKey(payload), gatewayManager),
