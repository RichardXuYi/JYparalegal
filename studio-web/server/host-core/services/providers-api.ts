...
import type { HostApiContract, ProviderModelMetaResult } from '@shared/host-api/contract';
import type { CompleteHostServiceRegistry } from '../main/ipc/host-contract';
import type { GatewayManager } from '../gateway/manager';
import type { ProviderConfig } from '../utils/secure-storage';
import { browserOAuthManager, type BrowserOAuthProviderType } from '../utils/browser-oauth';
import { deviceOAuthManager, type OAuthProviderType } from '../utils/device-oauth';
import { readModelEntryMetadata, removeProviderFromOpenClaw, saveProviderKeyToOpenClaw, updateModelEntryMetadata } from '../utils/openclaw-auth';
...
async function getModelsMeta(
  payload: ProviderPayload<'getModelsMeta'>,
): Promise<ProviderModelMetaResult> {
  const body = getPayloadRecord(payload, 'getModelsMeta');
  ...
  try {
    return await readModelEntryMetadata(providerKey, modelId);
  } catch (error) {
    logger.error('providers.getModelsMeta failed', error);
    return null;
  }
}

async function updateModelsMeta(
  payload: ProviderPayload<'updateModelsMeta'>,
  gatewayManager: GatewayManager,
): Promise<{ success: boolean; error?: string }> {
  const body = getPayloadRecord(payload, 'updateModelsMeta');
  const providerKey = typeof body.providerKey === 'string' ? body.providerKey : '';
  const modelId = typeof body.modelId === 'string' ? body.modelId : '';
  if (!providerKey || !modelId) {
    return { success: false, error: 'providerKey and modelId are required' };
  }

  const metadata: { contextWindow?: number; reasoning?: boolean } = {};
  if (typeof body.contextWindow === 'number' && Number.isFinite(body.contextWindow)) {
    metadata.contextWindow = body.contextWindow;
  }
  if (typeof body.reasoning === 'boolean') {
    metadata.reasoning = body.reasoning;
  }

  try {
    const result = await updateModelEntryMetadata(providerKey, modelId, metadata);
    if (result.updated) {
      logger.info(`Scheduling Gateway reload after updating model metadata for "${providerKey}/${modelId}"`);
      gatewayManager.debouncedReload();
    }
    return { success: true };
  } catch (error) {
    logger.error('providers.updateModelsMeta failed', error);
    return { success: false, error: String(error) };
  }
}
...
export function createProvidersApi(ctx: ProvidersApiContext): CompleteHostServiceRegistry['providers'] {
  ...
  return {
    ...
    cancelOAuth,
    submitOAuth,
    getModelsMeta: (payload) => getModelsMeta(payload),
    updateModelsMeta: (payload) => updateModelsMeta(payload, ctx.gatewayManager),
  };
}
