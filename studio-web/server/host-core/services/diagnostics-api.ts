...
import type { DiagnosticsStartupReportResult } from '@shared/host-api/contract';
  ...
  return {
    gatewaySnapshot: async () => {
      ...
      return {
        ...
        gateway,
        channels,
        'grandpoem-studioLogTail': await logger.readLogFile(DEFAULT_TAIL_LINES),
    ...
    startupReport: async () => {
      // Redacted terminal-failure artifact built by the gateway manager when it
      // entered the `failed` state; null when there is no terminal failure.
      const report = ctx.gatewayManager.getLastStartupReport();
      return (report ?? null) as DiagnosticsStartupReportResult;
    },
  };
}
