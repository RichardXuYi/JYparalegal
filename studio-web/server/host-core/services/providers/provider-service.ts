...
import { stripRuntimeProviderPrefix } from './provider-runtime-sync';
...
function inferProviderVendorIdFromOpenClawEntry(
  ...
  // Plan account keys: e.g. "qwen-124880ac" -> "qwen", "ark-a1b2c3d4" -> "ark"
  const planMatch = key.match(/^(.+)-[0-9a-f]{8}$/);
  if (planMatch) {
    const baseVendor = planMatch[1];
    if ((BUILTIN_PROVIDER_TYPES as readonly string[]).includes(baseVendor)) {
      return baseVendor as ProviderType;
    }
  }
...
export class ProviderService {
  ...
  async listAccounts(): Promise<ProviderAccount[]> {
    ...
    // 鈹€鈹€ openclaw.json is the ONLY source of truth 鈹€鈹€
      ...
      if (storeGroup.length > 0) {
        ...
        if (entry) {
          ...
          if (syncedAccount) {
            ...
            // Strip the runtime provider key prefix so `provider/model` refs from
            // openclaw.json never leak into stored accounts (self-heals stacked
            // prefixes from previously polluted data).
            const nextModel = shouldSyncSelectedModel
              ? (syncedAccount.model ? stripRuntimeProviderPrefix(syncedAccount.model, key) : syncedAccount.model)
              : kept.model;
  ...
  static buildAccountsFromOpenClawEntries(
    ...
    for (const [key, entry] of Object.entries(providers)) {
      ...
      // The default model is a full `provider/model` ref, so strip the runtime
      // provider key prefix before storing it on the account.
      let model: string | undefined;
      if (defaultModelProvider === key && defaultModel) {
        model = stripRuntimeProviderPrefix(defaultModel, key);
  ...
  // 鈹€鈹€ Internal silent variants 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  ...
  // 鈹€鈹€ New clean account-based public API 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  ...
  // 鈹€鈹€ Legacy public API (logs deprecation warning once per method) 鈹€
  ...
  // GrandPoem Paralegal Studio callers should use the internal/clean methods above.
