...
export function getOpenClawProviderKey(type: string, providerId: string): string {
  ...
  // Plan accounts use ids shaped as `${type}-[8hex]` as their unique runtime
  // provider key so each plan is an independent provider. Legacy UUID ids and
  // OAuth ids do not match this shape and keep their existing behaviour.
  if (new RegExp(`^${type}-[0-9a-f]{8}$`).test(providerId)) {
    return providerId;
  }
  return type;
}
...
export function getPrimaryModelId(model: string | undefined): string | undefined {
  if (!model) return undefined;
  const primary = model.split(/[，,\n]/)[0]?.trim();
  return primary || undefined;
}

/**
 * Strips a runtime provider key prefix from a model id (or comma-separated
 * list of model ids). Only the exact `${runtimeKey}/` prefix is removed, and
 * it is removed repeatedly so legacy values polluted with stacked prefixes
 * (e.g. `minimax-portal/minimax-portal/MiniMax-M3`) self-heal on the next
 * sync. Model ids that merely contain slashes (e.g. `deepseek-ai/DeepSeek-V3`)
 * are left untouched because only the exact runtime key is matched.
 */
export function stripRuntimeProviderPrefix(model: string, runtimeKey: string): string {
...
export function getProviderModelRef(config: ProviderConfig): string | undefined {
  ...
  const primaryModel = getPrimaryModelId(config.model);
  if (primaryModel) {
...
export async function syncUpdatedProviderToRuntime(
  ...
  if (isDefaultProvider) {
    const primaryModel = getPrimaryModelId(config.model);
    const strippedModel = primaryModel ? stripRuntimeProviderPrefix(primaryModel, ock) : undefined;
    const modelOverride = strippedModel ? `${ock}/${strippedModel}` : undefined;
    if (!isUnregisteredProviderType(config.type)) {
...
export async function syncDefaultProviderToRuntime(
  ...
  // repairs on-disk config written by earlier GrandPoem Paralegal Studio builds.
  try {
  ...
  if (!isOAuthProvider) {
    const primaryModel = getPrimaryModelId(provider.model);
    const modelOverride = primaryModel
      ? (primaryModel.startsWith(`${ock}/`) ? primaryModel : `${ock}/${primaryModel}`)
      : undefined;
    ...
    try {
      const defaultModelId = getPrimaryModelId(provider.model)?.split('/').pop();
