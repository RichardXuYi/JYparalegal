...
import { getGrandPoemStudioProviderStore } from './store-instance';

const PROVIDER_STORE_SCHEMA_VERSION = 2;

export async function ensureProviderStoreMigrated(): Promise<void> {
  const store = await getGrandPoemStudioProviderStore();
  ...
  // v0 鈫?v1: migrate legacy `providers` entries to `providerAccounts`.
  if (schemaVersion < 1) {
  ...
  // v1 鈫?v2: clear the legacy `providers` store.
