...
const AUTH_ERROR_PATTERN = /\b(unauthorized|forbidden|access denied|invalid api key|api key invalid|incorrect api key|api key incorrect|authentication failed|auth failed|invalid credential|credential invalid|invalid signature|signature invalid|invalid access token|access token invalid|invalid bearer token|bearer token invalid|access token expired)\b|閴存潈澶辫触|瑾嶈澶辨晽|璁よ瘉澶辫触|鐒℃晥瀵嗛懓|鏃犳晥瀵嗛挜|瀵嗛挜鏃犳晥|瀵嗛懓鐒℃晥|鎲戣瓑鐒℃晥|鍑瘉鏃犳晥/i;
...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[grandpoem-studio-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[grandpoem-studio-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (shouldFallbackFromModelsProbe(modelsResult)) {
    console.log(
      `[grandpoem-studio-validate] ${providerType} /models returned ${modelsResult.status}, falling back to ${apiProtocol} probe`,
    );
    if (apiProtocol === 'openai-responses') {
...
async function validateAnthropicHeaderKey(
  ...
  if (
    ...
    console.log(
      `[grandpoem-studio-validate] ${providerType} /models returned error, falling back to /messages probe`,
    );
