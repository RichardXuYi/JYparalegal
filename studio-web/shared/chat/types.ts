...
export interface ContentBlock {
  type: 'text' | 'image' | 'attachment' | 'thinking' | 'tool_use' | 'tool_result' | 'toolCall' | 'toolResult';
  text?: string;
  thinking?: string;
...
export interface ChatState {
  ...
  /** Thinking levels supported by the current session's model, as reported
   * by the gateway session row (`thinkingLevels` / legacy `thinkingOptions`).
   * Drives the thinking selector options; empty when the gateway reports none. */
  ...
  clearError: () => void;
  /** Drop all in-memory sessions/messages/caches when the account changes. */
