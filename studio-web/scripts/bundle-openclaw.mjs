...
function patchBrokenModules(nodeModulesDir) {
  ...
  for (const { rel, search, replace } of replacePatches) {
    ...
    if (!current.includes(search)) {
      echo`   ???  Skipped patch for ${rel}: expected source snippet not found`;
      continue;
    }
      ...
      for (const entry of entries) {
        ...
        if (entry.name === 'lru-cache') {
          ...
          try {
            ...
            echo`   ???  Failed to patch lru-cache at ${fullPath}: ${err.message}`;
          }
        } else {
          stack.push(fullPath);
        }
...
\t});`,
  ...
  for (const patch of replacePatches) {
    ...
    if (!target || !fs.existsSync(target)) {
      echo`   ???  Skipped patch for ${patch.label}: target file not found`;
      continue;
    }

    const current = fs.readFileSync(target, 'utf8');
    if (!current.includes(patch.search)) {
      echo`   ???  Skipped patch for ${patch.label}: expected source snippet not found`;
      continue;
    }
